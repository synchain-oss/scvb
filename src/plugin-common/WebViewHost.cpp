...
public:
    ...
    //   定谳与修法(放行只认 firstFrame + 信号到后再压一拍:tick 数 ∧ kRevealSettleMs 毫秒下界)只写在
    ...
    //   `kRevealSettleMs` 同理(取值与推导见 WebViewRevealGate.h 头注,这里不复述):它是
    //   一个**下界近似**,**不是**对「已经上屏」的观测 —— 观测在真机,本机量不到。
    // -------------------------------------------------------------------------
    //
    // -------------------------------------------------------------------------
    ...
    //   放行时序一字未动:WebViewRevealGate 的「等首帧 + kRevealSettleMs 下界」与本卡无关
