...
//   与观测相容 ≠ 被解释。input 那一档在 headless 上早 **200~370 ms**,比那 32 ms 的余量大
//   一个数量级 —— 但**那是 headless 的量级,真宿主上小得多**,见下面【真宿主 A/B】。
...
//   与 headless 上 monitor 那一档同一个量级,而 `kRevealSettleMs` 就有 32 ms。
//   ⇒ **本机这台真宿主上,修前也不该闪**,与 SL-420「本机复现不出 Input 的白」一致。
// ⇒ 口径:input 那一档是「**机制已在真宿主上确认存在**(修前信号确实可能早于首帧),
