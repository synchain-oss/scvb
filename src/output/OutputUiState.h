...
// 即拒载),header 那 6 个 u32 的偏移至今冻结、不许就地插字段;其后是**四级尾档**
// ([SL-416 R5] 更新:abi=1 时是「8 字节枚举尾少了才拒载」,SL-279 推到 16、SL-411 推到 28、
// SL-416 推到 52,下面这句才是今天的口径)——
//   · 尾长 0(= 旧 abi=1 payload,整段缺失)→ 全部尾字段回落默认;
//   · 尾长 8(`loudness_mode` + `center_slot_policy`)/ 16(再 + `applied.*`)/ 28(再 + `segmentation.*`)/
//     52(再 + `vad.*` 五项与 `transition_ramp_ms`)
//     —— 各档**内**少一个字节即拒载:`(0,8)` / `(8,16)` / `(16,28)` / `(28,52)` 四个空洞一律整块拒载
//     ([SL-279 复审] 起的纪律,SL-411 R8 把措辞收敛成「档内不许半截」);
//   · **≥ 52 之后**多出来的字节走 unknownTail 保留回写、并不拒载(任意长度都收)。
// 真源 = `src/core/state/OutputStateCodec.h` 头注与 `tests/core/test_output_session.cpp` 的
// 长度断言(78u / 24u+5u+52u);这里只记「它已经不是 8 字节那版口径」这件事,别再按 8 字节推理。
//
