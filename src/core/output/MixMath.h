...
// dB → 线性增益。
//
// ⚠ [SL-442] **别把这里的 `std::pow` 换成 `exp2`/`exp` 近似。**
// 本函数的**逐位输出**被一条**无容差**的判据依赖:`SL442-MIX-1`
// (tests/core/test_mix_source.cpp)断言「没画过 pan 曲线的工程,实时链输出与接入
// 角度域曲线 G 之前**逐位**相同」。那条成立的机制是:G≡0 时曲线增益因子恰为 1.0f,
// 而本函数的结果**逐字不变** —— 换近似式会改变这里的舍入,于是那条用例会红。
//
// 红的地方看起来与改动无关:**那个用例名里没有 "pow"、也不在这个文件里**。
// ⇒ 它不是 flake,**也不该靠给它加容差消掉** —— 容差一加,「逐位不变」这条保证就没了,
//   而那正是「老工程升级后声音一个字节不变」的全部依据。
