...
public:
    ...
    // 先按旧门控补拉、再换门控,这条缝就没了。
    //
    // 边界(评审建议⑥):这一发同样受 pullIncremental 的 kMaxBurstHops(每轨每拍 256 hop
    // ≈2.56s)约束。正常交互下布防时消息线程活着、积压 ≤256 hop,一发就清干净;只有
    // 「消息线程被阻塞 >2.56s 之后紧接着布防」这种边界才可能残留,那时残余仍会被 drainOnly
    // 丢掉。要彻底,得改成循环补拉到追上写头 —— 但那会在 [M] 上引入一个无界循环,
    // 代价比它挡的边界大,故留在这里如实登记,不做。
