...
// 之间重新分配能量,不改变「这一轨有多响」,所以轨道液柱不吃 pan;总线 L/R 取的是 **SCVB 自己
// 求和后的 accum**,即 busXfade **之前**的信号 —— 稳态下它就是送出去的东西,只有进出混音的
// 80ms 交叉窗口内会与总线实际输出有短暂差异(那几十毫秒里总线是干湿两路的等功率混合)。
// 刻意不取交叉之后的 buffer:那会把宿主的干信号也算进「本插件的总线电平」。
// 增益取本块起点的仲裁目标值(逐样本平滑的差异对电平表不可见)。
