...
// 口径提醒:panNow / volDb 是**播放头所在段**的段值(读回链 = `output/DistReadback.h`,与 Output
// 分布图逐条同源),不是 pan 车道在播放头所在列的采样 —— 后者是列中心点采样,列宽 = span/1024。
// [SL-363] 之前这里是「播放头精确时刻的曲线求值」;换掉的理由(空隙里两侧各走各的、ramp 窗口
// 最长 6 秒)写在 DistReadback.h 的头注。车道本身仍是曲线求值 —— 轨迹图那条线该有 ramp。
