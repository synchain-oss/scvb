...
// kMigrators[i] 将 abi=i+1 升到 abi=i+2。当前 abi=4,**三级都靠 OutputStateCodec 的「长度回退」**,
// 故三个迁移函数都是 no-op(不重写 CFGS payload):
...
// 再升 abi 时在此追加 migrate_4_to_5 等,并**新增** tests/golden/state/abi{N}.bin ——
// 旧的 abi{N-1}.bin **保留**作迁移基线,不是替换(abi1/abi2/abi3 就是这么并存的)。
...
inline constexpr std::array<MigrateFn, 3> kMigrators{&migrate_1_to_2, &migrate_2_to_3, &migrate_3_to_4};
