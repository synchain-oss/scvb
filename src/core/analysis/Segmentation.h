...
// [SL-414] 短自动段兜底并入(纯函数,就地改写)。该轨 **auto** 段中长度 < minSegmentMs 的段
// ⇒ 并入同轨相邻段:优先并入**前一段**(延长前一段的 t1),没有可并入的前一段就并入
// **后一段**(提前后一段的 t0);**值取被并入的那一段**(survivor 的 pan/vol 原样保留,
// 不重算)。循环直到该轨没有 < minSegmentMs 的 auto 段;整轨只剩一段时保留
// (S0 已保证 core ≥ min,孤段只会来自写回窗边裁剪,照旧由 [SL-399 R8] 那条账管)。
// origin != Auto 或 locked 的段**永不参与**(既不被并、也不吸收);相邻两侧都不是 auto
// 的短 auto 段原地保留(现实里不会出现:调用点 runAnalysisPipeline 回写层此刻全是 auto,
// 该分支是纯函数层的防御语义,由「用户段不动」用例钉住)。
// 不跨轨、不改区间图、不碰 §5 指派 —— 这是回写层「区间 × 活跃轨 → 段表」之后的
// 每轨兜底,只丢掉交叠区间里解出的那一小段双轨分槽值,两轨的覆盖都保住
// (统筹为什么不选区间层并入:规则 A/C 会级联吞掉邻轨整段,见 SL-414 定谳报告 §4.2)。
