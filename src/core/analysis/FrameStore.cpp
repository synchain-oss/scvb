...
void ChannelFrames::write(uint64_t hop, float kw_ms, float peak)
{
    ...
    //
    // 「素材换了、却既没打洞也没布防」那一档,分两半说清(#160 复审【重要】,别再照抄
    // 「交给指纹守望」这句 —— 它只对一半):
    //   · 采集 **OFF** 时换素材:Input 照跑提取并每秒上报 `fp_report`(04 §4.5),指纹守望
    //     拿它与**已采集特征**比,失配 ⇒ ⚠「上游音频与已采集特征不一致」,提示用户重采/重分析。
    //     这一档有人管,而且管法是**提示**、不是抹掉显示。
    //   · 采集 **ON** 时换素材:`fp_report` 在采集 ON 期间被显式抑制(`FeatRing.cpp` 的
    //     `capturing` 早退:此刻这一秒的特征正在被写成新基线,拿它跟自己比毫无意义),
    //     而基线本身又是现算自 `frameStore_` 的 —— 于是 ⚠ 永远不会亮,旧绿线会留到用户
    //     下次重新分析为止。**这一档是本卡相对 SL-206 的真空**:SL-206 那把钝器顺手盖住了它,
    //     代价是把最常见的「又听了一遍」砸了。取舍是明知的,该走的路是 `recaptureArm`
    //     (布防重采)或 `clearCoverage`(清除后重采)—— 两条都会经上面那两个条件作废判决。
    // 判据本身 O(log n)(n = 覆盖区间数,实测几十),与已经在做的 `coverage_.add` 同阶。
    if (vadInvalidateOnWrite_ || !hasHop(hop))
    {
