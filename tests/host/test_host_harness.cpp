          ...
          "[host][input][sl446]")
{
    ...
    // ⚠ [SL-446 第 2 轮] 这个场景里"实际持有"与"配置"**不分叉**(回滚成功,a 稳稳留在
    // kTestChannel 上,两个源头都是 kTestChannel)——不代表两个出口共用同一个值就是对的,
    // 见下面那条"加载期冲突"用例:那里两个出口**必须给出不同的值**,合回同一个源必红。
    //
...
TEST_CASE("SL-446(第 2 轮,集成,真 Processor):加载期冲突 —— 存档=配置、广播=实际,两者必须"
          "不同",
          "[host][input][sl446]")
{
    // ⚠ 这条判据的设计要点:**故意造一个两个出口必须给出不同值的输入**,不是各断各的。
    // 工程存了 channel 5,加载时 5 恰好被别的实例占着(启动期常见竞态,不是用户操作触发)——
    // 这种"配置了但没绑定"的场景下:
    //   · getStateInformation()(存档)必须是 5 ——那是用户配置的意图,不该被一次临时的
    //     加载期冲突擦掉(擦成 0 = 用户设置被永久抹掉,下次连"重试 5"的机会都没了);
    //   · bridgeTickSnapshot()(广播)必须是 0 ——那是这一刻的实际状态,说 5 就是撒谎。
    // 谁把两个出口合回同一个源,这条用例里两个断言不可能同时过——这正是本条要的效果。
    juce::ScopedJuceInitialiser_GUI juceInit;
    FakePlayHead ph;

    // occupant 先占住 channel 5,构成"加载时才发现被占用"的真实条件。
    ...
    // 构造一份"工程存了 channel=5"的存档字节(等价用户之前保存过、这次重新打开)。
    ...
    // victim:先 setStateInformation(等价宿主载入工程),再 prepareToPlay(等价宿主起播,
    // 触发真实 claim)——与 InputProcessor.cpp 头注写的"若已 prepare 立即 re-claim;否则
    // 由下一次 prepareToPlay 走 claim"顺序一致,走的是**从未 prepare 过**这条路径。
    ...
    victim.prepareToPlay(kSr, kBlock); // 真实 claim,撞上 occupant,失败(previousChannel==0,
                                       // 补偿式回滚不触发——与项目加载首次 claim 失败同形)。
    ...
    REQUIRE(scvb::state::decodeContainer(static_cast<const std::uint8_t*>(blob2.getData()), blob2.getSize(), chunks2) ==
            scvb::state::DecodeStatus::Ok);
    ...
    CHECK(loaded2.channelId == 5); // 存档:配置原样留着,没被这次临时冲突擦掉

    occupant.releaseResources();
    victim.releaseResources();
}

TEST_CASE("SL-446(第 2 轮,集成,真 Processor):setGroupId/ensureCtrlOpen —— 配置了但没绑定"
          "时,ctrl 段对齐仍按配置走,不当成未分配跳过",
          "[host][input][sl446]")
{
    // ⚠ 本条钉的是 ensureCtrlOpen() 里 `channelId_ == 0` 那个早退门槛——channelId_ 第 2 轮之后
    // 是**配置**镜像,"配置了但没绑定"时该门槛读到的仍是非零,ctrl 段的懒开/对齐必须照常进行,
    // 不能因为"这次没真的抢到"就被误当成 channel_id=0(未分配)跳过建段。
    // 观测点:一旦 ensureCtrlOpen() 因门槛判断错误提前 return,段就没开——不管 Output 那边广播
    // 写了多少次,bridgeTickSnapshot().broadcastValid 永远是 false(段未打开时 lastBroadcastValid_
    // 恒 false,见 HOST L-4a 头注)。反过来 broadcastValid 变 true 就直接证明段真的开了、
    // 门槛判断走的是"有配置"那条路,不是"未分配"那条路。
    // ⚠ 故意用 setChannelId()(不是 setStateInformation())触发分叉:setStateInformation()
    // 自己对 ctrl_ 有一条独立的、直接读解码出来的 s.channelId(天然就是"配置",不经 channelId_
    // 镜像)的 changeGroup 分支(见 InputProcessor.cpp:510-522)——那条路径提前把 ctrl 段开好了,
    // 会把本该由 ensureCtrlOpen() 单独决定的这件事悄悄做掉,使这条判据测不出真正的 bug(已实测
    // 确认:用 setStateInformation() 构造这条用例,即使把镜像源改回 boundChannel(),这里仍然
    // 假绿)。setChannelId() 完全不碰 ctrl_,ctrl 段只能靠 ensureCtrlOpen() 懒开,才是真正在测
    // 这个门槛。
    juce::ScopedJuceInitialiser_GUI juceInit;
    FakePlayHead ph;
    ...
    REQUIRE(victim.setChannelId(5) == scvb::input::InputClaimState::kConflict); // 配置=5,实际未绑定

    Rig::pumpMessages(300); // 让 out 的 25Hz Timer 把广播区写出去,victim 自己的 Timer 也顺带跑

    const auto snap = victim.bridgeTickSnapshot(); // 内部先调 ensureCtrlOpen()
    CHECK(snap.channelId == 0); // 广播的"实际持有"仍是 0(与上一条判据同口径),不受本条影响
    CHECK(snap.broadcastValid); // 段真的开了——channelId_ != 0(配置)这条门槛没被误判成 0
...
// 50/120/500/1000/2000 → 段数单调不增` 直接构造 `PipelineConfig`,**绕过了 `startAnalysis`
// 装配那一跳**。删掉 `cfg.vad.minSegmentMs = runtime_.segmentationMinSegmentMs` 这一行,
// 那条 core 用例照样全绿 —— 本仓对这类「守得住零件、守不住那一跳」有判例
// (`HOST SL263` 头注逐字)。本用例就是补那一跳。
//
// **素材是设计出来的**:机台默认的 `capture()` 是 640ms 爆发 + 427ms 间隙,
//   ① 那一种素材在 [50,2000] 的值域里只有**一档**可丢(640ms 只对 >640ms 的门限失效,
//      50/120/500 三档一段都丢不掉)⇒ 断不出「逐档递减」这条方向;
//   ② 427ms 间隙经 padding(120+200)之后只剩 107ms < mergeGap(150)⇒ P4 会把相邻爆发
//      并成一段,连「有几段」都测不出来。
// 所以这里自造:爆发 3/8/20/45/80/**220** block(≈ 32 / 85 / 213 / 480 / 853 / 2345 ms,
// 1 block = 512/48000 s ≈ 10.67ms),中间垫 60 block(≈ 640ms)静音 —— 640 > 320 + 150,
// 三条后处理都不会把它们并回去。各档门限各自吃掉前几个。
//
// ⚠ **最后那个 220 block(≈ 2345ms)的爆发是 [SL-398] 加的,不是凑数**:`min_segment_ms`
// 的新上限是 **2000ms**,素材里必须存在**长于 2000ms 的段**,那一档才有**非空产出**可断言。
// 只加一档 2000 到旧素材(最长 853ms)上会得到 `n2000 == n500` —— 那不是「2000 没生效」,
// 而是 `applyAnalysisSegments` 的 `src.empty()` 空判(`OutputProcessor.cpp` 的 [SL-399 R16]:
// 「整条时间线上一段都没有」⇒ 段表**保持原样**)把旧段表原封兜住了:**全丢光在 host 面是 no-op**。
// 2026-09-13 的删除式验证第一版正是撞在这上面(旧素材下 500/2000 都是 2 段,看着像没传下去),
// 所以这里的断言是 `n500 > n2000` **且** `n2000 >= 1`,不许把「全丢光」当成判据。
//
// **实测段数**(取自本用例 `SL391-COUNTS` 那行 `UNSCOPED_INFO` 的实跑输出,`-s` 可见,不是推的):
//   50ms   → 6 段   120ms → 4 段   500ms → 3 段   2000ms → 1 段
// 断言只要求**逐档递减**,不写死这几个数:素材一动它们就会变,而本用例要钉的是
// 「这个参数到底有没有传下去」,不是「这份素材恰好切出几段」。
//
// ⚠ [SL-398] 那一档(2000)本用例**够不到 `OutputEditor.cpp` 的 `jlimit(50,2000)`** ——
// editor 依赖 WebView2,不在 host 套件的 TU 清单里(下面那段注释讲得比这里细)。
// 它钉的是**运行时那一跳**:2000 一路传到 VAD 没被谁夹回 500(夹回去则 2000 档与 500 档
// 段数相等 ⇒ `CHECK(n500 > n2000)` 红)。桥面那一跳的缺口照旧登记在下面,不因本档消失;
// 真实桥面那一跳的夹取由 `smoke-tab3-interactions.mjs` 的 (c) 格(源码级对拍)+ 它的
// 删除式验证(D2a:把 `jlimit` 上界改回 500 ⇒ 该格红)兜。
//
// ⚠ 断言写成**逐档递减**,不是「不相等」:几档一起塌成 1 段(= 接线断掉、恒取默认值
// 的那种形态之一)也满足「有变化」,那种绿是假的。
//
// 删除式:① 删掉 `OutputProcessor.cpp` 里 `cfg.vad.minSegmentMs = runtime_.segmentationMinSegmentMs`
// ⇒ 四档恒取 `VadParams` 的默认 120 ⇒ 段数四档相等 ⇒ 本用例红;
// ② 把那一行改成「> 500 就取 500」(模拟上游多一道旧上限的夹取)⇒ 2000 档与 500 档相等 ⇒
// `CHECK(n500 > n2000)` 红(2026-09-13 实跑核过,见 PR 记录)。
// ---------------------------------------------------------------------------
          ...
          "[host][analyze][segmentation][SL391]")
{
    ...
    // [SL-398] 最后那个 220 block 的爆发(≈ 2345ms)是本卡加的:它是唯一能活过 2000ms 门限的段,
    // 于是 2000 那一档有**非空产出**可比(为什么必须这样,见头注那段「全丢光在 host 面是 no-op」)。
    ...
    const std::size_t n2000 = countAt(2000); // [SL-398] 值域上限那一跳

    UNSCOPED_INFO("SL391-COUNTS 段数 50/120/500/2000 = " << n50 << " / " << n120 << " / " << n500 << " / " << n2000);
    ...
    CHECK(n50 > n120);
    CHECK(n120 > n500);
    // [SL-398] 上限档:素材里那条 2345ms 的爆发是**唯一**活得下来的段 ⇒ 段数落在 1。
    // 这一条同时是「2000 没被夹回 500」的判据 —— 夹回去则与 500 档同数,本行红。
    // 另配一条 `n2000 >= 1`:上面那条在「2000 档全丢光、空判保住旧段表」时也可能成立,
    // 而那种形态**证明不了 2000 生效**(它连 VAD 那一跳都没量到)。
    CHECK(n500 > n2000);
    CHECK(n2000 >= 1u);
}
...
// T50 与 T500 逐字节相同 ⇒ 第 ③ 步找不到 S,只能 `REQUIRE(false)`(这是从 SL-391 头注的
// 实测推出来的,本用例没有跑过默认素材那一版)。分级爆发才有「T50 有、T500 没有」的段。素材
// 配方与理由逐字见
