...
if (tracked === null) {
    ...
    // ⚠ 这一段必须落在下面那道汇总判负**之前**:它是往 `engineRot` 里塞发现的
    //    (第 1 轮初版塞的是 `listRot`,第 2 轮拆出 `engineRot` 时**这句没跟着改** —— 记一笔,
    //    免得下一个人 grep `listRot` 找不到写入点),而那道判负会把 `listRot` 与 `engineRot`
    //    一起消费掉。放到判负之后 = 塞进去的东西没有任何人再看,门禁照绿 ——
    //    本卡初稿就是这么写的,复审第 1 轮的注入实测把它逮出来了。
