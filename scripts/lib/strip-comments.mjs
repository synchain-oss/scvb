...
//     ⚠ [SL-330c] **自测只钉得住本模块这一侧**:表里「本模块不剥」「本模块抛错」两档各有
//     一格(见 `--self-test`),改宽那行字符集会红。而「**真 PowerShell** 开不开注释」那一侧
//     的每一条都是**实测快照,一格都钉不住** —— Node 里跑不到 pwsh。这不是纸面顾虑:
//     上一版 `x =#c` / `y]#c` 写错时,34 格**全绿**。所以动这张表的任何一行,
//     都要**拿 pwsh 7 的 `Parser::ParseInput` 重测那一行**,别只改字。
