...
//     拿 pwsh 7 的 `Parser::ParseInput` 逐个实测过:真 PowerShell 判的是**这个 `#` 落在哪个
//     token 里**,不是前一个字符 —— `foo#bar` / `a+#c` / `.\a#c` / `-Force#c` 整个是**一个
//     Generic token、不开注释**(本模块与它逐字一致);而 `$a#c` / `1#c` / `'x'#c` / `"x"#c`
//     / `x =#c` / `y]#c` 在真 PowerShell 里**都开注释**,本模块**不剥** —— 那才是少剥的实处。
//     字符级近似**做不到逐字复刻**(它得知道有没有起过一个裸词),要根治只能上真词法器,
//     那正是 SL-330c 的方向。还有一档方向相反:裸词里含 `<#` 的写法(`a<#c`,真 PowerShell
//     整个是一个 Generic token)会被本模块当块注释开头 ⇒ 扫到文件尾抛错,是**假红、会出声**。
