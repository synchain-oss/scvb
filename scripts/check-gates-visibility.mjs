...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    //   ⚠ [SL-330b] 下面三条 ⚠ 里拿「**行尾注释**」当病灶的那几个形态,今天在进到这条正则
    //   之前就已经不存在了:`ps` 现在由逐字符扫描器剥,**行尾注释连同行首注释一起剥掉**。
    //   三条判据**照留** —— 它们各自独立于剥离器成立(剥离器把某处读岔、或将来有人把这一步
    //   换回文本级,判据仍拦得住),而留着的代价是零。下面几段照原样保留,因为它们记的是
    //   这一族**怎么一层层长出来的**,不是今天还活着的病灶。
    //   ⚠ 尾部还要排掉 `#`:当年 `ps` 只剥**整行**注释,行尾注释原样留着,于是
