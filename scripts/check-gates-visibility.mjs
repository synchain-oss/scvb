...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    //   末尾 `\b` 的理由与下面 `$draftsAllow` 那条逐字相同([SL-318] 一起补):收在词尾的
    //   变量名断言,改名成 `…X` 会命中前缀照样绿 —— 一族三条,不留一条没补的。
    if (!/-f\s+\$smokeLabel\s*,\s*\$smokeFlaky\b/.test(ps))
        fail(
    ...
    //   [SL-318] 末尾的 `\b` 不是装饰:这条断言原先以变量名收尾、**后面什么都不要求**,
    //   于是把 `$draftsAllow` 改名成 `$draftsAllowX`(= 拼进标签的是个空变量)它照样命中前缀、
    //   照样绿 —— 本卡的删除式第 12 格就是这么把它照出来的。同段另外两条(`= 0` / `+=`)
    //   后面都还要求一个非词字符,天然没有这个问题;只有这条收在词尾。
    if (!/-f\s+\$parityLabel\s*,\s*\$draftsAllow\b/.test(ps))
        fail(
