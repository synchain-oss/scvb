...
//   ① **生产端**:每一套 `smoke-*-page.mjs` 的两个「有依赖但失败」出口必须走 `browserFailed()`。
//      每套各手抄一份 —— 「只改其中几份」「照旧模板新起一套」都是本仓有前科的形态;
//      **这里不记套数**:执行面按 glob 收,记在注释里的数没有任何东西会让它变红
//      (SL-370 加第七套时原文那个「六」就已经错了)。
...
//     ⚠ 别读成「整条链唯一的少剥」:剥离器自己那一侧另有几档,**清单与逐条实测在
//     `lib/strip-comments.mjs` 的头注边界段**,这里**只指路、不复述** —— [SL-330c] 起改成
//     这样,因为上一版在这里抄了一份夹具清单,而其中两个串在 [SL-339] 被实测推翻之后
//     **只改了那边、没改这里**,这一句就成了主线上的假话。摘录一旦复述具体形态就会漂;
//     指路不会;
...
for (const s of pageSuites) {
    ...
    if (i < 0) {
        ...
        //   跨行块注释则短得多。今天各套的余量都很宽(实测:`browserFailed` 与 `noBrowser`
...
if (!fs.existsSync(WF)) fail("找不到 .github/workflows/format.yml");
    ...
    // (别把它读成「整条链唯一的少剥」:剥离器自己那一侧另有几档,清单与逐条实测在
    // `lib/strip-comments.mjs` 的头注边界段 —— **只指路,不在这里复述**,理由见本文件头注
    // 同一条旁边那句。)
    const yml = read(WF)
        .split("\n")
        .filter((l) => !/^\s*#/.test(l))
