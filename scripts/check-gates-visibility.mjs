...
//     还能顶替真 `echo`,是**注释形态**里今天唯一留着的那一档漏判(另有一档少剥不在注释上:
//     `lib/strip-comments.mjs` 不解析 PowerShell 双引号串里的 `$( … )`,今天零处用点);
...
for (const s of pageSuites) {
    ...
    if (i < 0) {
        ...
        // ⚠ [SL-330b] 本段与上面那两条带上界的窗口(`{0,900}` / `{0,200}`)**都跑在剥后
        //   文本上**,而剥法是「**只留换行**」:注释正文没了,每条注释仍占掉一个换行。
        //   与换实现之前(整行连同换行一并删掉)相比,注释密集的段落会**长一点点**,
        //   跨行块注释则短得多。今天六套的余量都很宽(实测:两个函数体各约 250 字符、
        //   `chrome.on("error"` 与调用同一行、`if (!targets)` 的下一行就是调用),
        //   方向也是假红且三条文案都点了「或长过匹配窗口」—— 写在这里是因为
        //   「换剥法会动窗口余量」这件事不写下来就没人会想到。
...
if (!fs.existsSync(WF)) fail("找不到 .github/workflows/format.yml");
    ...
    // 还能顶替真 `echo` —— **注释形态**里今天唯一留着的那一档漏判。
    // (别把它读成「整份检查唯一的少剥」:`lib/strip-comments.mjs` 自己的边界表还列着
    // PowerShell 双引号串里的 `$( … )` 不解析,方向同样是少剥;那一档今天 `gates.ps1` 里
    // 零处用点,而这一档是活的。)
    const yml = read(WF)
        .split("\n")
        .filter((l) => !/^\s*#/.test(l))
