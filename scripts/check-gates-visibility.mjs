...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    if (!/\$draftsAllow\s*=\s*0/.test(ps))
        fail("`$draftsAllow` 没有初始化 —— gate 3i 的 [ALLOW] 没有独立计数");
    ...
    if (!/-f\s+\$parityLabel\s*,\s*\$draftsAllow/.test(ps))
        fail(
            "`$draftsAllow` 没有被拼进 `$parityLabel` —— [ALLOW] 只会出现在滚屏里," +
                "而跑完 gates 的人看的是汇总表:与 SL-297 在 3e 上堵的是同一个洞。",
        );
    // ★ 本卡特有的第二条:计数模式必须**锚在行首**。成功消息里也会提到这两个信号的名字,
    //   按裸 `\[ALLOW\]` 数的话 **0 条放行也会数出 1** —— 一个恒定高报 1 的豁免计数,
    //   恰好废掉「豁免必须显形且数得准」。SL-313 那轮 gates 在主线上实测到回显 2 行就是它。
    //   注意只收紧**计数**:回显仍要按裸标记匹配(它要把 [ALLOW] 与 [BASE] 两类都打出来)。
    const cnt = /\$draftsAllow\s*\+=[^\n]*\n?[^\n]*/.exec(ps);
    if (cnt && !/\^\\s\*\\\[ALLOW\\\]|\^\\\[ALLOW\\\]/.test(cnt[0]))
        fail(
            "`$draftsAllow` 的计数模式没有锚在行首(应含 `^\\s*\\[ALLOW\\]`)—— " +
                "裸 `\\[ALLOW\\]` 会把成功消息里提到的那句也数进去,0 条放行恒报 1。",
        );
}
