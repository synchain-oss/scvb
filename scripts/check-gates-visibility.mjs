...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    //   ALLOW 与 WARN 两处计数都从它实例化。所以这里也只读一次、对两个标记各跑一遍夹具:
    //   两份逐字相同的字面量的坏处不是重复,是**只改一份** —— 那时「WARN 收紧了、ALLOW 没有」
    //   在汇总表上看不出任何差别。下面另有两条断言钉住「用点确实走的是它」,
    //   否则谁在某一处把字面量写回去,这里照样绿(读到的还是那份好定义)。
    const fmtM = /\$markerCount\s*=\s*'([^']+)'/.exec(ps);
    if (!fmtM)
        fail(
            "gates.ps1 里读不到 `$markerCount = '<模式>'` —— 三个标记的计数口径没有一处定义," +
                "也就无法验证它到底数不数得对",
        );
    else {
        ...
        for (const c of cases) {
            const src = fmtM[1].replace("{0}", c.mark);
