...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    //   扫的是**全文件的字符串字面量**,不是 `console.log(...)` 的实参 —— 本卡第一版就只扫
    //   实参,而那条被违反的成功消息恰好提成了 `OK_MSG` 常量,于是注入实测**照样绿**:
    //   「只扫我以为它会出现的地方」正是本仓反复栽的那一类,判据要扫的是文件,不是我的预期。
    ...
    for (const sc of gate3iScripts) {
        ...
        for (const lit of src.matchAll(/"((?:[^"\\]|\\.)*)"/g)) {
            const i = lit[1].search(/\[(?:ALLOW|BASE)\]/);
            // i === 0 是**真信号**(`"[ALLOW] …"`)或带缩进的真信号,放行;
            // i > 0 且前面不全是空白 ⇒ 是**散文里提了一嘴**,那就会冒充信号。
            if (i > 0 && !/^\s*$/.test(lit[1].slice(0, i)))
                fail(
                    `${sc}:字符串里有一句散文提到了方括号标记` +
                        `(「${lit[1].slice(Math.max(0, i - 12), i + 9)}」)—— ` +
                        `gate 3i 按裸 \`\\[ALLOW\\]|\\[BASE\\]\` 回显成功输出,` +
                        `这句会冒充一条真信号;提到 ALLOW / BASE 时去掉方括号`,
                );
        }
    }
}
