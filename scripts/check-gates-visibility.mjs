...
//   [SL-315] 起 ② 段里多守一族**同构**的接线:gate 3i 的 `[ALLOW]` 放行 —— 同一个失效方向
//   (打了标记只在滚屏闪一行、汇总表照写干净的 PASS),所以判据也逐字同构。
...
if (!fs.existsSync(GATES)) fail("找不到 scripts/gates.ps1");
    ...
    //   成功输出按裸 `\[ALLOW\]|\[BASE\]` 回显,所以任何一句「见 [ALLOW] 行」这样的散文
    //   都会冒充一条真信号 —— 回显多一行,若哪天有人把计数改回裸匹配就再加一条恒 1。
    //   这条不变量到 [SL-315] 为止已被违反**三次**(#197 第 5 轮的 `[BASE]`、`379de23` 的
    //   `[ALLOW]`、以及本卡第一版**在本文件自己身上**),三次都只有散文守着 —— 所以给它一个
    //   跨文件的执行者,而不是每个脚本各写一份手抄自测。
    //   扫的是**三种字符串字面量**(`"…"` / `'…'` / 模板串),不是 `console.log(...)` 的实参。
    //   这一条我连栽两版,两版是**同一个洞降了一层**:第一版只扫实参,而被违反的那句恰好提成了
    //   `OK_MSG` 常量 ⇒ 注入实测照样绿;第二版只扫双引号,而 `check-native-paths.mjs:185` 的
    //   成功行本来就是模板串(`console.log(\`② 正反例: …\`)`)⇒ 在它旁边补一句带方括号的模板串,
    //   守卫照样全绿。**「只扫我以为它会出现的地方」正是本仓反复栽的那一类** ——
    //   判据要扫的是文件,不是我的预期。
    ...
    for (const sc of gate3iScripts) {
        ...
        const src = read(f)
            ...
            .filter((l) => !/^\s*(\/\/|\/?\*)/.test(l))
            .join("\n");
        // **逐个标记看它前面是什么**,而不是先把字符串切出来 —— 切字符串要真 tokenize:
        // 三种引号互相嵌套(本文件里就有 `` `…"…"…` `` 这种),用正则择一吃掉会**错配**,
        // 第三版就是这么假红的(报到一段横跨代码的「字面量」上)。
        // 判据换成:一个方括号标记合法,当且仅当它**开一条字符串**(往回跳过空白后是引号)。
        // 那正是「真信号」的形态(`"  [ALLOW] #" `);其余一律是散文里提了一嘴 ⇒ 会冒充信号。
        // 拼装写法(`"[" + "ALLOW] 行"`)天然不命中 —— 源码里根本没有连着的标记。
        // 行内尾注释没剥 —— 与头注那张边界表同一口径。
        for (const m0 of src.matchAll(/\[(?:ALLOW|BASE)\]/g)) {
            const before = src.slice(0, m0.index).replace(/[ \t]+$/, "");
            if (/["'`]$/.test(before)) continue; // 开头即标记 = 真信号
            fail(
                `${sc}:字符串里有一句散文提到了方括号标记` +
                    `(「${src.slice(Math.max(0, m0.index - 12), m0.index + 9).replace(/\n/g, " ")}」)—— ` +
                    `gate 3i 按裸标记回显成功输出,这句会冒充一条真信号;` +
                    `提到 ALLOW / BASE 时去掉方括号(要写成真信号就让它开一条字符串)`,
            );
        }
    }
}
...
console.log(
    "check-gates-visibility 通过:生产端 " +
        ...
        "gates 3i 的 ALLOW 放行有独立计数、计数锚在行首、且已接进汇总标签;CI 侧 rc=3 分支在且不判红。",
);
process.exit(0);
