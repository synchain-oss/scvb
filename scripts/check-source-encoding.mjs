        ...
        while (i < n && depth > 0) {
            ...
            if (c === '"') {
                ...
                while (i < n) {
                    ...
                    if (code[i] === "\n") break;
                    if (bad === null && code.codePointAt(i) > 127)
                        bad = code[i];
                    i++;
                }
                if (bad !== null) {
...
function selfTest() {
    ...
    expect(
        "cjk-adjacent-multiline",
        `fprintf(stderr,\n  "[SL-000] ok${nl}"\n  "${cjk}${nl}",\n  1);`,
        1,
    );
    // 括号嵌套:实参里有函数调用时不能提前收工。
    expect(
        "cjk-after-nested-call",
    ...
    if (got.length !== 1 || got[0].line !== 3) {
        fails.push(
            `lineno:期望第 3 行 1 条,实得 ${JSON.stringify(got.map((h) => h.line))}`,
        );
    }
...
if (files.length === 0) {
    console.log(
        "  [FAIL] src/ 与 tests/ 下一个 C/C++ 源文件都没扫到 —— 空集合会让本判据恒真",
    );
    process.exit(1);
}
...
for (const f of files) {
    ...
    violations.push(
        ...findViolations(src, relative(REPO_ROOT, f).split(sep).join("/")),
    );
}
...
if (calls === 0) {
    ...
    console.log(
        "  [FAIL] 全仓一处 printf 族调用都没扫到 —— FAMILY 多半被改坏了,判据面为空即恒真",
    );
    process.exit(1);
}
...
if (violations.length > 0) {
    ...
    for (const v of violations) {
        console.log(
            `    ${v.file}:${v.line}  ${v.fn}(… ${v.text} …)  首个非 ASCII 字符:${v.char}`,
        );
    }
    console.log(
        "    修法:运行期文案改 ASCII,要说的中文留在注释里(见 tests/support/exclusive_guard.h)。",
    );
    process.exit(1);
}

console.log(
    `  printf 族字面量全 ASCII:扫了 ${files.length} 个源文件、${calls} 处族内调用,0 处违规`,
);
