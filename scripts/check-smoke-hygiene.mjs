...
//       `bodyAfter` 与 `braceBody` 取出的 `text` **原样含注释**,所以体**内部**写一句
//       `// 旧写法是 process.on(sig, teardown)` 同样能顶替真调用 —— 与上面第一条
//       (字符串包含判断)是同一个洞往里挪一层,不是新洞。
//       **没有修**:`bodyAfter` 那支能顺手攒一份去注释的文本,但带花括号那支走
//       `braceBody`、它根本不跳注释 —— 只修一半会让「体内注释不参与判定」这句话
//       只对一半形态成立,比明说更坏。要修就两支一起上 AST。
