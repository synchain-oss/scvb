...
//        被外部目录锁包着)所以堵住了整批。**[SL-301] 之后 3e 自己持锁,一套挂死
//        重新会堵住全场** —— 这条纪律因此从「防自己白跑」升级成「防拖垮全机」。
...
//     · **文本匹配判断**:三种 handler 形态都靠**文本**认(形态二/三用 `includes`,
//       **形态一是整份文件源码上的正则**)⇒ 一句 `// teardown 在别处做` 的注释也能
//       顶替真调用。**能顶替的位置见下面第五条** —— 形态一那份是**全文件**,
//       不限于 handler 体(原文写「三种靠 `includes`」「handler 体里」,两个量词
//       各差一格,而差掉的恰好是最宽的那一份);
...
//     · **注释只被排除在「深度计数」之外,不被排除在「判定文本」之外**:
//       参与 D 档判定的**每一份**文本都原样含注释,**共四份**:`parenSpan` 给的
//       `head.text`(形态二)与 `call.text`(形态三)、`bodyAfter` 给的 `body.text`
//       (体带花括号时这一份就是 `braceBody` 切出来的那段;`braceBody` 在 D 档不另算
//       一份),以及**形态一直接拿整份文件源码 `s` 去正则匹配**。
//       **最后这一份最宽,别把可顶替的位置读成只有前三段**:文件里**任何位置** ——
//       头注、被注释掉的旧写法、说明文字 —— 写一句字面的
//       `// process.on("exit", teardown)`,形态一当场判过,三段切出来的文本一份都不用碰。
//       实测:五条退出路径的注册**全部只写在注释里、一个真 `process.on` 调用都没有**的
//       样本,D 档判「无问题」。(该样本仍带一个真的 `function teardown()` —— 否则
//       在「D. 没有 `teardown()`」那一支就被判掉,压根走不到形态一。
//       ——点名那一支、不写它在第几行:引用者与被引者同在本文件,加几行注释就把它推走了
//       (`:322` 正是这么在写下它的同一个提交里失效的),本文件头注自己立过这条规矩。)
//       今天各套真文件里的 `process.on(` 全是真调用、无注释形态,所以这是
//       **潜在缺口而不是现网假绿**;自检矩阵也逮不到它 —— 那两格假注释写的是
//       `process.on(sig, teardown)`,`sig` 是变量,打不中形态一那条要求字面量带引号的正则。
//       与上面第一条(字符串包含判断)是同一个洞往里挪一层,不是新洞。
//       **没有修**:`bodyAfter` 那支能顺手攒一份去注释的文本,但 `braceBody` / `parenSpan`
//       两支根本不跳注释 —— 只修一部分会让「注释不参与判定」这句话只对一部分形态成立,
//       **比明说更坏**。要修就三支一起修。
...
//   今天各套逐个核过:`waitFor` 体与三个 handler 体花括号/圆括号平衡、串里无裸括号、
//   收尾段无 `http://` 之类的串 —— 但别以为它比实际更严。
//   ⚠ [SL-330b / SL-339 复审] 上面这几条**原来写着「要根治都得上 AST,对这道门禁不划算」,
//   那个前提已经不成立**:同一族(串里的 `//` 被当注释、注释冒充代码)在
//   `scripts/lib/strip-comments.mjs` 里用**词法级逐字符扫描**根治了,**没有建语法树**,
//   而且是本文件这几支要的那种粒度(哪一段是串、哪一段是注释)。
//   本文件没跟着换是**范围**问题不是**可行性**问题 ——
//   `commentEnd` / `commentBoundary` 是两个**强度不同**的口径,`braceBody` / `parenSpan` /
//   `bodyAfter` 三支各有自己的切法与上界近似,换过去要连它们的自测一起重做。
//   别照着这几句去推「这一族只能上 AST」;真要换,读那个模块的头注 ——
//   **这件事挂在 SL-340,本卡有意不动**。
//
...
// 「折行不受影响」也只是今天恰好成立,别把它写成保证。今天各套都不是那种写法。
...
// 抽之前这些检查只在真文件上跑过,而真文件恰好各套写法完全一致 —— 等于判据的大部分
...
// 各套真文件今天的写法(形态一 + 两个带花括号的 for 循环)—— 正例基线。
const REAL_SHAPE = [
...
const SELFTEST = [
    ...
    [
        ...
        // 与「循环头与体之间的注释里写着注册,真体不收尾(复审第 4 轮)」那格同形,
        // 只是把行注释换成跨行块注释 —— 行注释那格打不中这条路径。
        // (点名而不写格号:插格会让格号漂,本文件头注自己立过这条 —— 去 grep「格数以」。)
        [
            'process.on("exit", teardown);',
            'for (const sig of ["SIGINT", "SIGTERM"])',
            "    /* 见",
