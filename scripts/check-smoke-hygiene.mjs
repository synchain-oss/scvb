...
//       参与 D 档判定的**每一份**文本都原样含注释,**共三处**:`parenSpan` 给的
//       `head.text`(形态二)与 `call.text`(形态三)、以及 `bodyAfter` 给的
//       `body.text`(体带花括号时,这一份就是 `braceBody` 切出来的那段;
//       `braceBody` 在 D 档不另算一份)。所以体**内部**、循环头里、乃至
//       `process.on(…)` 的实参之间写一句 `// 旧写法是 process.on(sig, teardown)`
//       都能顶替真调用 —— 与上面第一条(字符串包含判断)是同一个洞往里挪一层,不是新洞。
