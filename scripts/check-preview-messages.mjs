...
export function collectMessages(code) {
    ...
    for (const m of code.matchAll(
        /(?<!function )evalLanded\([\s\S]*?,\s*(?:"([^"]*)"|`([^`]*)`)\s*,?\s*(?:\{[^{}]*\}\s*,?\s*)?\)/g,
    ))
 ...
 * 通配同时接受 `…` —— 表里就是用它占位的。返回 `null` 表示这条消息开头的字面量太短,
 * 不足以当查找锚点(例如整条都是插值),那种消息不参与本判据。
 */
...
function run() {
    ...
    const section = norm(guide.slice(at));
    ...
    for (const { raw, where } of messages) {
        ...
        if (!head) continue; // 开头几乎全是插值,当不了查找锚点
