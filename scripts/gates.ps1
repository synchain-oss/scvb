...
elseif ($smokeFiles.Count -eq 0) {
  ...
  # 会回 2 的是**三套**页面级冒烟(都要一个无头 Chrome/Edge):smoke-monitor-page.mjs、
  # smoke-output-stale-page.mjs(SL-177 过期提示,04 §4.5)与 smoke-output-dist-page.mjs
  # (SL-203 分布图补间)。执行面按 glob 自动收,加一套不必改这里 —— 这行只是给人读的。
  # 为什么单列一档:
