...
if (-not $ipcLockOk) {
  ...
  if (-not $cmakeCmd -or -not $ctestCmd) {
    ...
    if ($ctestBlocked) {
      ...
      #
      # [SL-320] 这个数**不再是任何一套的上界,只是兜底**。各套的上界已经全部写进
      # `tests/CMakeLists.txt` 的 TIMEOUT 属性(口径真源在那边 `scvb_tests` 上方那段):
      #     scvb_tests / scvb_params_tests / scvb_plugin_common_tests / scvb_input_bridge_tests → 30
      ...
      # ⚠ 上一版这段把 `scvb_ipc_tests` 写成「属性 900」——**是 host 才对**,ipc 至今没有属性,
      # 它正是今天唯一还落在这条兜底上的一套(实测 ~30s,300s 是十倍余量)。
