...
# 「顶层条目全覆盖」也降级成警告。所以本地绿不等于这几档验过,它们以 CI(ubuntu)为准。
if (-not $nodeCmd) {
  ...
  foreach ($sc in @('check-bridge-parity.mjs', 'check-curve-parity.mjs', 'check-design-box.mjs', 'check-native-paths.mjs', 'check-smoke-hygiene.mjs', 'check-gates-visibility.mjs')) {
    ...
    if ($LASTEXITCODE -ne 0) {
      ...
      # 另有「顶层条目全覆盖」一档,也是一行。而 Windows 上 `grep` 通常不在 PATH 上
  ...
  #     每行仍各对应一档,行数与档数**仍是 1:1** —— 这才是这半句要说的。
  #   · 原文说 check-bridge-parity 的 `warn()` helper「已定义、当前零调用」,并举
  #     `:1229` 那处「真跳过却裸 warns.push、这里数不到」当反例 —— 那四处**已改走 warn()**
  #     (现 :1234 / :1272 / :1288 / :1300),裸 push 只剩 helper 自己那一处,反例没了。
