...
// [SL-330a] 收窄侧的**执行者**:读一次计一次。
// 此前守「收窄调用点必须用这个常量」靠的是**源码 needle**(在自己的源码里找那一串字符),
// 而那条路三轮下来成了「needle 守 needle」—— 判别串失步、锚点过期、注释里的连续字面量把
// `indexOf` 引到自己身上,四种失效**全是那一格自己造的**,不是被守对象出的问题。
// 换成**行为断言**:调用点每读一次这个常量,计数加一;自测跑一次生产入口
// (`allowList` 的正文放行支)后断言计数真的动了。把调用点写回内联字面量时计数不动 ⇒ 当场红。
//
// ⚠ 边界(照实说,别让人以为它更严):
//   · 它钉的是「**这次运行里读到过**」,不是「源码里引用了」——所以它挡得住「写回内联
//     字面量」和「写回 `new RegExp("^" + …source + "$")`」这两条(两者都不经过本函数),
//     但挡不住「读了却不用」(读一次丢掉、另用一个自己造的正则)。那种写法要 AST 才拦得住,
//     记在 SL-330c 的同族里,**不在本卡**;
...
function allowList(block, headText = ALLOW_HEAD) {
    ...
    for (let j = head + 1; j < lines.length; j++) {
        ...
        // [SL-330a] 经 `anchoredRe()` 取常量,不直接引用 —— 那是「调用点真的用了这个常量」
        // 这条判据的执行者(读一次计一次,自测断言增量;见常量定义处那段)。
        if (headText === BODY_ALLOW_HEAD && !anchoredRe().test(m[1])) {
            wrongForm.push(m[1]);
            continue;
        }
...
if (selfTest) {
    ...
    try {
        ...
        //     所以另加一条断言,钉住调用点确实用了这个常量。
        // [SL-330a] 这条断言**从「源码 needle」换成了「行为探针」**。
        //   旧法:在自己的源码里找 `<常量名>.test(m[1])` 那一串字符,并计数 `=== 1`、
        //   看位置、三串全从常量名派生 —— 三轮下来成了「needle 守 needle」:判别串失步、
        //   锚点自己过期、注释里的连续字面量把 `indexOf` 引到自己身上,**四种失效全是那一格
        //   自己造的**,不是被守对象出的问题;而且拼装出来的串必然躲开全局改名(IDE rename /
        //   sed),那时报的是「被写回内联」而调用点一点没内联 —— 指错方向。
        //   新法:调用点经 `anchoredRe()` 取常量,**读一次计一次**;这一格跑一次生产入口
        //   (`allowList` 的正文放行支)再比计数增量。把调用点写回内联字面量、或写回
        //   `new RegExp("^" + …source + "$")`,两者都不经过 `anchoredRe()` ⇒ 计数不动 ⇒ 当场红。
        //   判据面从「源码里的字符」挪到了「这次运行里的行为」,于是它**不再需要守自己**:
        //   改名、注释里怎么写、这一格自己的话术长什么样,都不影响它。
        // ⚠ 边界(照实说):它钉的是「**读到过**」,不是「读了之后真的用了那个结果」——
        //   「读一次丢掉、另用一个自己造的正则」这种写法它挡不住,那要 AST 才拦得住,
            ...
            check(
                ...
                anchoredDelta === 0
                    ? "收窄调用点没有读 BODY_REF" +
                          ...
                          "三种都不经过 anchoredRe()(所以增量都是 0),区别在还引不引用那个常量:" +
                          "① 引用着,真源放宽时收窄跟着动;②③ 连常量都不引用," +
                          "真源放宽那天收窄不会跟着动"
                    : "读取者不止一个(增量 " +
                          anchoredDelta +
                          ")—— 多出来的那个会替生产那一处顶着:" +
