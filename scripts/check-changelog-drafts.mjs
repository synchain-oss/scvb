...
if (selfTest) {
    ...
    try {
        ...
        // 三条 needle 全部**从一份常量名派生**,不手写第二串([SL-328] 复审第 2 轮):
        // 常量改名时,改的人必然会改 `anchoredCallSite`(不改就 0 命中、当场红 —— 它有执行者),
        // 而派生保证判别串跟着一起动。此前判别串是手写的第二串:它只在 0 命中那一支被读到,
        // 常量改名后 `constCallAt` 恒 -1 ⇒ 三支判别静默退化成两支 ⇒ 真的 `m` → `match` 改名时
        // 报的仍是「写回内联」——**本卡开题要修的毛病原样回来,而注释还写着「分三支」**。
        // 拼装(而不是写成连续字面量)也不能省:整串会命中本格自己的注释与话术,
        // 于是 0 次那一支永远选「needle 过期」——「扫描器扫到自己」那一族(实测踩过)。
        // ⚠ **边界(统筹 SL-328 收口裁定):源码级 needle 是权宜之计,它自身没有上位判据。**
        // 这一格守的只有一件事 —— 收窄调用点要用那个常量。为了守住它,本格自己引入了三条
        // needle,而每条 needle 又成了下一条要守的东西:三轮复审下来,判别串失步、注释描述
        // 了不存在的判据、锚点自己过期、注释里带连续字面量把 indexOf 引到自己身上 ——
        // 四种失效全是**这一格自己造的**,不是被守对象出的问题。
        // 派生把前几条闭合了,代价是把失步集中到下面那一串上:它按「别扫到自己」的要求必须
        // **拼装**,于是全局改名(IDE rename / sed)**必然扫不到它**;那时 0 命中且判别串也
        // 落空 ⇒ 报「被写回内联」,而调用点一点没内联 —— 本卡开题那句「第二次红指错方向」
        // 在**改名**这条路上仍会复现。今天这棵树上三支话术各自对得上真因,这条是残留边界。
        // 真正的执行者应当在 AST / lint 层(「这个常量必须被引用」+「禁止用字面重建它」),
        // 源码 needle 只是它的穷人版 —— 记在 **SL-330**,不在本卡。
        // 所以:**别再往这一格加第四条 needle**。同族意见一律转 SL-330。
        const anchoredConstName = "BODY_REF" + "_ANCHORED_RE";
        const anchoredConstCall = anchoredConstName + ".test(";
        const anchoredCallSite = anchoredConstCall + "m[1])";
        const anchoredHits = selfSrc.split(anchoredCallSite).length - 1;
        // 判别串**要看位置** —— 不是 `.includes`(`>= 1`),也不是纯计数。两者都挡不住这一条:
        // 自测里写一处 `…test("111")`(不是 `m[1]`,上面那个计数不受影响、无人察觉),生产那处
        // 写回内联 ⇒ 判别串**仍恰好命中 1 次** ⇒ 报「needle 过期,调用点没坏」,而真因是写回
        // 内联,话术还明确劝人别去看调用点,比原状更坏(实测:只加计数时那一格仍报错方向)。
        // 所以判据是「那一处落在**自测块之前**」= 它真是生产调用点。
        const constCallAt = selfSrc.indexOf(anchoredConstCall);
        // `=== 1` 只保「全文件恰一次」,保不了「那一次在生产侧」:同一个 PR 里自测加一处、
        // 生产写回内联,命中数仍是 1 ⇒ 照绿(门禁只看合并后那棵树,中间态没有执行者)。
        // 所以再断一次**位置**:那唯一一次要落在自测块之前。
        ...
        // 第三条 needle(自测块锚点)也会自己过期 —— 那个块头被重构成带比较的写法、
        // 提成函数、或多加一个条件,都会让它落空。单独一格报,
        // ⚠ 这段注释里**不能**写出块头的连续字面量:写了它自己就会被 indexOf 命中,
        // 真块头被重构掉时锚点仍「找得到」(指到这条注释上)⇒ 本格静默失效。
        // 实测踩过:第一版注释里带了那一串,Y5 那格照绿。
        // 别和「落在自测块之内」挤进同一句括号:那正是本卡开题要拆的写法(复审【建议】)。
        check(
            selfTestAt > 0,
            "找不到自测块锚点 `" +
                selfTestHead +
                "` —— 下面那条「调用点要在生产侧」的位置判据没有参照物了(块头被重构过?)," +
                "把本格的锚点跟着改",
        );
        check(
            anchoredHits !== 1 || selfTestAt <= 0 || callSiteAt < selfTestAt,
            "那唯一一处 `" +
                anchoredCallSite +
                "` 落在自测块之内 —— `=== 1` 只保「全文件恰一次」,保不了「那一次在生产侧」:" +
                "同一个 PR 里自测加一处、生产写回内联,命中数仍是 1",
        );
        check(
