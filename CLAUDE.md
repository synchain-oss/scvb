...
- **native 路径清单([SL-283],单一真源 = `build-vst3.yml` 的 `detect-native` 步里那条 `NATIVE_RE`)**:`src/` / `tests/` / `cmake/` / `CMakeLists.txt` / **`web/`** / `third_party/` / 四个钉版文件 / `build-vst3.yml` 自身。子 PR 碰到其中任一就照跑全量。
  ...
  - **新增顶层目录会当场红([SL-286])**:上面那几档保的都是「这条正则别被改坏」,保不住**另一个方向** —— 构建面长出一个正则没覆盖的新目录。比如将来加 `res/` 并让 CMake 消费它,**加它的那个 PR 自己一定会编**(它必然动 `CMakeLists.txt`),当场看不出问题;而此后**只碰 `res/` 的子 PR 静默判 native=false**。所以门禁还会穷举 `git ls-files` 的顶层条目,要求每一个都落进**三个归宿之一**:`NATIVE_RE` 的命中面、`check-native-paths.mjs` 里那张**显式** non-native 清单、或(天生混合的目录,如 `.github/`)`MIXED_TOP` + `MIXED_EXPECT`(混合条目连**命中集合**也逐条钉死,否则它就是个豁免口)。三处都不沾即判红,报错直接给出路。**加任何新的顶层目录/文件,都要在这三处之一登记一笔** —— 清单**不留通配兜底**,写成「其余都算 non-native」这道门当场退化成恒真。
