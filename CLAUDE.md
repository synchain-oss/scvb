...
- 一律经 `pwsh scripts/gates.ps1`(06 §5.1 的 gate 1–8,另含十个子档:`3b` gitleaks、`3c` reuse lint + `check-spdx.ps1`、`3d` 设计盒真源、`3e` web smoke、`3f` 文档真源、`3g` IPC 契约文档对拍、`3h` 字体子集覆盖、`3i` 桥面/曲线/设计盒/native 路径/冒烟写法/预写条目对拍、`3j` 隐私、`3k` 字体保留名)。子档只增不减,以 `gates.ps1` 的 `Set-Gate` 为准。
...
- **native 路径清单([SL-283],单一真源 = `build-vst3.yml` 的 `detect-native` 步里那条 `NATIVE_RE`)**:`src/` / `tests/` / `cmake/` / `CMakeLists.txt` / **`web/`** / `third_party/` / 四个钉版文件 / `build-vst3.yml` 自身。子 PR 碰到其中任一就照跑全量。
  ...
  - 清单**只加不减**要谨慎:漏一条 = 改了它却没编,且是静默的。这条正则**有自己的门禁**:`scripts/check-native-paths.mjs`(docs-truth 里跑),做正反例断言 + **删除式验证**(逐个删掉正则的一个分支,都必须有正例变红)+ **两个引擎命中集合对拍**(CI 侧是 `grep -E`,门禁自己是 JS RegExp)。改 `NATIVE_RE` 时同步改那里的正反例清单 —— 只改正则不改用例,门禁会直接红给你看。
  - **新增顶层目录会当场红([SL-286])**:上面那几档保的都是「这条正则别被改坏」,保不住**另一个方向** —— 构建面长出一个正则没覆盖的新目录。比如将来加 `res/` 并让 CMake 消费它,**加它的那个 PR 自己一定会编**(它必然动 `CMakeLists.txt`),当场看不出问题;而此后**只碰 `res/` 的子 PR 静默判 native=false**。所以门禁还会穷举 `git ls-files` 的顶层条目,要求每一个都落进**三个归宿之一**:`NATIVE_RE` 的命中面、`check-native-paths.mjs` 里那张**显式** non-native 清单、或(天生混合的目录,如 `.github/`)`MIXED_TOP` + `MIXED_EXPECT`(混合条目连**命中集合**也逐条钉死,否则它就是个豁免口)。三处都不沾即判红,报错直接给出路。**加任何新的顶层目录/文件,都要在这三处之一登记一笔** —— 清单**不留通配兜底**,写成「其余都算 non-native」这道门当场退化成恒真。**边界要说清:这一档只兜顶层**,已在 non-native 清单里的目录**内部**长出构建边不在它能力内(典型形态:哪天 CMake `add_custom_command` 消费 `scripts/foo.py`,此后只改 `foo.py` 的子 PR 依旧静默 `native=false`)—— 别把它读成「不再静默漏编」的全称保证。
...
- **可选依赖:无头 Chrome / Edge**(T46 起)。`web-preview/tests/smoke-*-page.mjs` 这几套(经 CDP 驱动真页面:Monitor 投影面、Output 过期提示面、Output 分布图补间面)用得到,**没装不算失败** —— 该套回退出码 **2**,`scripts/gates.ps1` 的 Gate 3e 与 CI 的 web-smoke job 都把 2 记成 SKIP / `::warning::` 而不判红(理由写在那两处的注释里:web-smoke 是 required check,为一个可选依赖判红会卡住仓库里每一个 PR)。**[SL-297] 另有退出码 3 = 「浏览器在,但这一次没起来 / 没连上」**(spawn 失败或 CDP 端口没在预算内开出来)——**同样不判红**,但 gates 打的是 `[FLAKY-SKIP]` 并**单独计数进汇总标签**、CI 出的是措辞不同的 `::warning::`。两档必须分开读:把「装着浏览器、这一次没跑成」当成「没装浏览器」,会让一次瞬时超时**无声地**抹掉整套判据而汇总照写全 PASS(SL-293 多轮 gates 实测撞到三次)。装了才跑得到「页面真的执行起来」那一层断言 —— node 侧的其余各套都不执行页面 JS,建议装。
...
**同契约不够 —— 时序也要同形**(SL-357):mock 的**状态回声默认异步**,写的回执先到、`scvb.state` 后到一拍(250ms = 4Hz 一帧),并按 `staleFullEchoEvery` 每 N 次写插一帧**过期的全量帧**。理由:同步 mock 会让「写完立刻读 store」的产品代码永远绿,到真桥才炸 —— 用户 v5.6.7 报的「第一下只出横幅、第二下才弹窗」就活在那个差里,而当时没有任何一套冒烟拦得住。
