...
| `stereo-mixed` | mono+stereo 混存:stereo 轨带 ST、Input 侧那一轨**显式**设 `participate_in_auto_pan=false`([J83] 起默认一律 `true`,本 fixture 要的是「用户手动排除某轨」的渲染面)、每轨 width 旋钮可用。**降级变体 `&loop=none`** 见上 |
