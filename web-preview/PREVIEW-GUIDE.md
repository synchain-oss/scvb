...
| `scenario` | `empty` `connected` `misaligned` `conflict` `occupied` `group-switch` `no-output` `print-guard` `first-run` `recapture-armed` | 场景 → fixture 的映射 + 快照覆写;表外值回落 `fifteen-tracks` 并 warn「待接线」 |
