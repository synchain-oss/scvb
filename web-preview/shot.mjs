...
if (args.has("help")) {
    console.log(
        [
            ...
            "        壳页一直没离开「未就绪」态、壳页上找不到 .pv-status(只要 URL 指向壳页就判,给不给 --url 都一样),",
            "        以及落地后页内求值抛错)。",
...
// 注入失败、壳页一直没离开「未就绪」态、壳页上找不到 `.pv-status`(**只要 URL 指向壳页就判**,
// 给不给 `--url` 都一样)—— 后两种是 [SL-333] 加的 —— 以及落地后的页内求值抛错)。
...
` + "   预览页要先起服务:pwsh web-preview/serve.ps1",
    ...
    // ⚠ [SL-333] **`__D` 一共注两次,真正被用到的是第四道判据之后的那一次**(壳页可能在
    //    等待窗口里重导航 iframe,把这一次存下的 Document 作废,理由写在那一处)。
    //    这一次**故意留着**,不是死重:它先跑,才能让「`__D` 注不进去」在「壳页没起来」
    //    之前报出来 —— 反过来的话,人先看到的是壳页那句(带壳页原话),而真正的病因是注入。
    //    代价是一次页内求值,毫秒级。删它之前先想清楚这个报错顺序(复审第 4 轮问过一次)。
    ...
    //    [复审第 3 轮] 前提**不是**「给没给 `--url`」,而是「**这个 URL 是不是壳页**」:
    //    文件头 `:21` 推荐的用法里就有 `--url=…/web-preview/input.html` —— 给了 `--url`、
    //    打的却正是壳页,按「给没给」分档会把那条推荐用法排除在守卫之外。反过来那半
    //    (没给 `--url` ⇒ 一定是 `buildUrl()` 拼的壳页)是硬的,这半不是。
    //    壳页路径形状从 `ROLES` 派生,不手抄第二份名单。
    ...
    const SHELL_PATH_RE = new RegExp(
        "^/web-preview/(" + ROLES.join("|") + ")\\.html$",
    );
    const isShellUrl = (u) => {
        try {
            return SHELL_PATH_RE.test(new URL(u).pathname);
        } catch {
            return false;
        }
    };
    ...
    // ⚠ [SL-333 复审第 3 轮] **等完之后必须重注一次 `DOC_PRELUDE`。**
    //    `DOC_PRELUDE` 是**一次性快照**:`window.__D` 存的是注入那一刻的 Document 对象,
    //    不是每次求值现取。而上面这个等待窗口(本卡新加,最长 SHELL_SETTLE_MS)**专门在等的
    //    那件事**——壳页的注入重试——恰好会 `frame.src = url` 重导航 iframe,把旧 Document
    //    作废。于是状态落到 `"1"` 放行之后,下面的 `clickIn` / `--tab` / `--lang` / `--click`
    //    全打在一个已经作废的文档上:选择器静默无命中,而汇总里看不出任何异常。
    //    改动前这个窗口只有一瞬,是本卡把它拉长的 —— 所以这是本卡的副作用,不是旧账。
    //    **无条件重注,不写成「等过才重注」**:那会把正确性挂在「这一轮有没有真的等」这个
    //    前提上,而本文件今天已经栽过一次同样的不对称(守卫挂在「第一次读」上)。重注是幂等的。
    await evalLanded(cdp, DOC_PRELUDE, "重新注入 __D(真源文档)失败");
