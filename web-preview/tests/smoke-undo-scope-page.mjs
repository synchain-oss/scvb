...
// ⚠ 这份清单此前只列到 ⑥,而正文已经跑到 ⑧ —— **同一份头注里有两张清单**(这张 + 下面
//   删除式网格那张),复审轮 4 只同步了后者。改判据面时**两张都要同步**。
//
...
//     ⑧ 造出跨宏任务的桥桩之后 **C8 当场转红**(红在 (k10))。
//     ⇒ 「钉不住」常常只是**尺子还没造出来**,不是真的不可达 —— 下这个结论之前
//     先问「我的夹具有没有把那个时间窗做出来」。
//     (至今仍未被任何一格钉住的只剩 commit() 里那道消费点守卫,原因写在它自己的
//      注释里:在途窗内 UI 的版本号本身是陈旧的,属契约层残余。)
...
try {
    ...
    newBucket("远端换版本");
    {
        ...
        // (这句说的是**⑥ 这一段**的局限,不是说那两格至今没人管:
        //  C6 现由本段 ⑦ 钉住,C8 由 ⑧ 的 (k10) 钉住 —— 两格今天都会红。)
