...
//   .18s(病态)   181.8ms     4.90%                    11.74px / 29.06px
//   no-liquid     0.0ms       14.03%                   −2.35px(柱头反冲到线外)
//   no-both       0.0ms       12.95%                   0.00px / 0.00px
//   short-33      36.4ms      11.70%                   6.00px / 25.98px
//
// 【SL-204 用户拍板 2026-08-27】终取 **short-33**(落地 = `--dur-meter-tube: 33ms`)。
// SL-191 先取的是 no-both(渲染逐帧 == 模型,最干净),但上行随之变成单帧跃迁
// (中位 89.9px/帧),用户判起跳太硬 —— 遂改取「一个事件间隔」的补间。
// 真源现状(`--variant=current`)复测:
//   阶跃滞后 36.3ms · 渲染触线 11.98%(模型 11.75%)· 该触线帧渲染Δ **中位 0.00px**
//   上行 192 帧 / 中位 10.98px 每帧(no-both 是 32 帧 / 中位 89.88px)⇒ 起跳已软化
//   下行 中位 2.94px 每帧 ⇒ 该平滑的那一侧没丢
//
