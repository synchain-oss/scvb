...
// 四张卡各 1.3 次/秒翻转。J93 三裁:①灭侧加迟滞 ②图表卡摘出 ③三张参数卡改徽标。
{
    ...
    //     (a17) 第一项变 false、(a18) 首次熄灭落在 ~+924ms ⇒ 两条都红;
    //   • 记账反向(`transportPlayingAt` 连停走帧也刷新时刻):窄档永远进不去 ⇒
    //     (a16) 第二项、(a19) 的「真停也熄」全红;
    //   • 去抖窗口放宽到 >= 停走档:(a15) 红,(a19) 的 900ms 上界也跟着红。
    check(
    ...
    // 「去抖那一档当家时也不会更晚」。
    {
        ...
        check(
            ...
            `(a19) ★ 「真停」三格都在停走后 ${HE.HOST_ECHO_RELEASE_STOPPED_MS}ms 内熄灭` +
                `(实得 ${JSON.stringify(stopped)};-1 = 一直没熄 = 去抖把停播那一半修坏了)`,
        );
    }
