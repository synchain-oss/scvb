...
);
{
    ...
    // 「`% 17` 抽稀真的生效了吗」——**两条路径自比**,不牵扯任何封顶数值(复审第 6 轮)。
    // 上一版写的是默认档 `framesAtCap === 0`,那又隐含一根「封顶 > 默认档每帧条数(~29)」
    // 的钉子:真到那一天它红出来会说「抽稀没生效」,而抽稀好好的 —— 失败文案指错方向,
    // 比不断还费事。抽稀生效这件事的定义就是「默认档比满档少」,与封顶取多少无关。
    check(
        seenBy.false < seenBy.true,
        `默认档(${seenBy.false} 条)必须严格少于满档(${seenBy.true} 条)—— ` +
            "两者相等就说明 `% 17` 抽稀没生效,满档那一支不再是独立路径," +
            "而上面两条断言仍会双双绿着",
    );
}
