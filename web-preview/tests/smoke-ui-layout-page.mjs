...
//         把 syncStale 里那句「没有待观察的写就只留 badge、不弹框」的早退删掉,
//         **且把紧跟的 token 计算改成 null-safe**(不改的话删完只会在那一行抛
//         TypeError —— 那是崩溃不是「闸没了」,框反而弹不出来,本条照绿),
//         则 C8s 那格红(实测:`C8s 但弹窗**没有**弹` 变红)。
//         ⚠ 同一注入还会连带把 C8r 那一段(SL-348 范围档)打红若干格,且**是抖动的**
//         (两轮实测一轮红一轮不红):闸没了以后模态框会在别的段里乱弹,扰动那边的
//         点击与焦点。连带红不是本条的判据面,别据它读结论。
