...
//  且停摆 / 宿主节流 / 换组 / 旧版 4Hz Output 这些降级路径一个都没消失。)
{
    ...
    // ★ 删除式:删掉建器里的 `setProperty` 或 `classList.add`,下面两条分别当场红。
    {
        ...
        // 渲染完全不变(类照打、线照对齐)、页面级也全绿,**降级形态却惄惄退回
        // 「图顶横线」**;谁把 `.dist-plot.has-zero-line` 选择器写错,那一页的线直接不画。
        // [SL-353] 页面级那边的覆盖面要分两半看:量**页面自己那张图**的只有
        // `smoke-output-dist-page` 的 `⑪ [SL-280]`(只跑 Output);同文件的 `⑫ [SL-353]`
        // 两页都跑,但量的是它自己造的沙箱 —— 选择器写错它接得住(拿不到
        // `display:block` 就红),删掉 `display:none` 那一档它**接不住**(降级形态
        // 只在「变量喂不到」时现形,而沙箱里变量总是喂到的)。两条都在「改错时会怎样」这一档。
        //
        ...
        for (const page of [
            ...
            // 页面级那边,量**页面自己那张图**的只有 `⑪ [SL-280]`(只跑 Output);
            // `⑫ [SL-353]` 两页都跑,但它量的是自己造的沙箱、**不看页里那个**
            // `<div class="dist-plot__zero">` 在不在。加上缺无头浏览器时整套退 SKIP,
            // 「元素本身在不在」在 Monitor 侧仍是零判据。
