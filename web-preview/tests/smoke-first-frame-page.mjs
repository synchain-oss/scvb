...
for (const role of ["input", "output", "monitor"]) {
    ...
    // 所以不给容差,改成**同源夹逼**:页面那次读数必然落在 [最近一次 tick 时刻, 桩收信号时刻]
    // 之间。**成立性**只靠「tick 时刻是在页面读数之前写下的」:页面读 now() 到调 postMessage
    // 是同一段同步代码,tick 插不进来。rAF 按注册顺序回调(桩的 tick 每帧都最先注册)决定的是
    // 下界有多**紧** —— 页面的 signal() 跑时本帧 tick 已经跑过,下界就是本帧的;这条一旦失效
    // (或走 2.5s 保险路),下界退到更早一帧,这两格**不会变红,只会静默变松**,宽度只在下面
    // 那行 log 里看得到、没有断言看着它(保险路上区间本来就会合法变宽,所以不能断宽度)。三个时刻与
