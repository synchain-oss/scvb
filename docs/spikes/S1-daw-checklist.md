...
## 0d. 修复版重跑须知(2026-08-16 → v8)

本包为**修复版 v8**。修复链:v5 及更早 mono⇄stereo 重建越界崩溃(转储同址实锤)→ v6 段恒按 stereo 容量创建 + 实际容量校验 → v7 RIP/undo 复制体释放 slot 后 Input 自愈重认领 → **v8:自动选槽塌缩根治**。
**v8 根因(v7 实测发现)**:`claimInput` 的「同 pid 重认领」让每个新实例都把已占 slot 判「认领成功」,自动选槽永远停在 ch1 —— 15 个 Input 全部挤在同一个 channel(导出只剩单轨内容、Output 听感与 ref_A 完全不同)。v8 新增 `claimInputAuto`(自动认领专用,不抢已占 slot),15 实例各占一个 channel(压测 multiclaim 场景 + 单测回归,24223 断言全绿)。

- **v8 装机后复位一次(必做)**:关 Cubase 重开(加载 v8)→ 15 个 Input + Output **全部删除 → 从插件浏览器全新插入**(这次自动选槽正确)→ 播放挂 csv 确认 **ch01..ch15 全活跃、out_mask=32767** → 保存工程。之后判据依赖 15 轨的格才有效。
- **强烈建议【新建工程】**(同 v4/v5):旧工程插件实例可能已被宿主禁用,判据失真。
- §1 通用准备**从头重做**(同 v4/v5);**无需重跑**:C-10、C-12、C-14(重启后健康态已完成,结果有效)。
- **开 DAW 前自检(推荐)**:工具包内 `pwsh tools\stress-run.ps1`,5 场景全 PASS 再开始。
- **崩溃证据自动收集**:`pwsh tools\collect-crash.ps1 -Setup` 注册转储(一次性),测试时挂 `pwsh tools\collect-crash.ps1 -Watch`;崩溃自动归档 dmp + journal。
- **v8 要重跑(复位后)**:C-3(导出 null)、C-5(听感)、G-1a、G-2、G-9(b)与 **G 系列剩余**(G-5/G-8/G-10 转 REAPER 机);G-2 与重导后的 `ref_Aprime` 对 null。
