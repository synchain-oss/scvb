...
| 链 | 用例 | 真链路 | 末端判据 | 反向验证 |
|---|---|---|---|---|
| **C1** | `HOST SL-231:真 Output 的配置与曲线经 viz 段发布,只读方逐字段读回` | 真 `Rig`(Output+Input)+ 真 Win32 viz 段;写方是**真 Output 的 `publishVizFrame()`**,不是 peer | 只读方 `VizPlane::attachReadOnly()` + `read()` 读回的帧:`seq` 为偶(未撕裂)、`sampleRate`、`versionActive`、`label[ch]` 逐字节(UTF-8)、`leadMask` 对位、车道非全哨兵、`playheadSamples` 随走带推进 | ① **对照轨**(一个字段都没动)必须是空 label + `leadMask` 位清 + 车道全哨兵 —— 挡住「装配把同一份值填满所有轨」;② **邻组** `attachReadOnly()` 必须失败 —— 挡住「publisher 发错组、断言靠残段恰好成立」 |
