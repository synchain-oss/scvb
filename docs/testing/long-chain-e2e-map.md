...
3. 有**反向验证** —— 把链中段掐断(或换成一个已知会坏的配置)时,同一条断言必须变红。
   第 3 条不是形式要求:`tests/core/test_viz_plane.cpp` 的 `publishedInOneSecond` 头注记着教训
   —— 第一版反向验证没加抖动,于是它测不出自己要防的东西,
   **一条测不出自己要防的东西的断言等于没有**。
...
| D7 | **采集 ON 期间整条比对是哑的**(设计如此:这一秒的特征正被写成新基线)—— 且机会**一次性消耗**,那一遍带采集的重播会把基线刷成改后素材 | ✅ | `HOST SL-239:采集 ON 期间提示是哑的,且机会一次性消耗`(两条负向 + 一条正向防空转);用户侧的可见性由 web 横幅 ⑨ 承担,见下 |
...
| **整链 × 真重开** | **D1→D6 经 F1→F4** | **✅ e2e(SL-239 新增)** | **`HOST SL-239:工程重开(全新实例)后上游改动仍须翻出 stale`** —— 反向 = 同一份工程重开后播**同振幅**素材必须不报(它同时是 FEAT 回灌保真度的判据:落盘存的就是量化后的 int16 dBq,回灌差一位就整轨误报) |

⚠ **SL-225 那批用例有一处盲区,SL-239 补上**:它们的「重开工程」是**同实例 `setStateInformation`**,
FrameStore 的基线全程留在内存里 —— `FEAT → FrameStore` 那一跳(链 F 的 F4b)**从没被指纹链走过**。
换成 SL-226 的真路径(`Rig` 析构 + 全新实例)后仍然是绿的,但在补上之前,「基线随工程回来了」
这件事对指纹链而言是**假设**而不是护栏。新增跨链用例时值得对着这一条自查:
**你的「重开」是真的把被测物销毁了,还是只把 state 又灌了一遍?**

⚠ **本表 ✅ 的口径提醒(SL-239 立)**:D1→D6 全绿**不等于**用户看得见提示。上面每一条 e2e 在问
`captureStale` 之前都显式 `setCaptureEnabled(false)`,而用户的自然流程把采集留在 ON(终验清单
A1/B13 没有一句叫他关)——于是链全绿、用户侧功能等于不存在。D7 与 web 横幅 ⑨ 就是为这条缝补的。
