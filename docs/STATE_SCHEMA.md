...
- **`CFGS.capture_enabled` 恒写 `0` —— 采集态不随工程走**([J91] 2026-08-30 用户批准;**推翻** [SL-225] 的「存用户自选值」口径):采集是一次**录制动作**,不是工程设置。
  [SL-225](#146)当时只堵了一种来源(布防替用户开的那一下不许落盘),但让「重开后采集莫名开着」的路不止那一条 —— 用户自己开着采集保存、或打开一份 v5.6 期间已被污染的旧工程,重开后照样是 ON。而采集 ON 期间 Input 一条 `fp_report` 都不发,于是 04 §4.5 的上游改动 ⚠ **对用户而言根本不存在,且查不出原因**(SL-239 用户实测两轮)。
