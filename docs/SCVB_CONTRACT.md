...
| 返回 | `{ok:true}` 或 `{ok:false, reason:"badArg"}` |
| 语义 | 写 state `analysis.loudness_mode` / `analysis.center_slot_policy`(03 文档层新增字段,**零修宪**)。**零 gesture**。义项与默认值以 05 §2.4 / J69 为准:`loudness_mode` 默认 `kw_integrated`(K 加权段积分;词条正名不用 "LUFS-S"),`center_slot_policy` 默认 `priority_queue`(按优先级排队)。改后**不自动重分析**,UI 显示「改后需重分析」注记 —— [SL-278] 该注记**两项都有**(此前只有 `loudness_mode` 那一块)。三档数学定稿以 02 §4.3 / §5.6 为准。<br>[SL-279] **`analysis.applied.{loudness_mode, center_slot_policy}`**(§1.1 / §2.1 载荷)= **上次全量分析所用**的那一档;`analysis_settings_stale`(03 §6.3)由 UI 按「当前 ≠ applied」**逐项**派生,两枚注记各挂各的控件。写入口仍只有本函数(它**只改当前值**);`applied.*` 只由一次**覆盖整条已采集时间线的全轨重算**前移,且随分析结果一起进撤销栈(SL-209)。今天有**两个**这样的来源:① §1.6 `analyze` 的 `"all"` 档(含缺参形态);② §1.18 / §1.19 松手档触发的**自动重分段**(改完 VAD / 分段参数 300ms 后)。两者调的是同一条范围推导,因此**都以 `global.range` 为 `follow` 档为前提**:`daw_loop` / `manual` 档下这条推导取的是 `global.range`,重算只覆盖范围内 —— 范围外的段仍是旧口径,所以**不前移**,注记保留(**唯一能让这一档真正落到整条时间线上的出路,是把 `global.range` 切回 `follow` 再分析**(把当前档**改回 `applied` 的那个值**同样会让注记灭 —— `analysis_settings_stale` 是逐项相等比较 —— 但那是**放弃这次改档**,不是让它生效);判据看的是**档位**、不是覆盖面:`manual` 档下即使把范围拖成 `[0, 已采集末端]`,`analyzeAllRange` 仍走范围支、仍不前移)。这是产品决定:注记的语义是「段表未按当前档分析过」,范围档下它亮着是**真话**。同理,§1.6 的**对象形 scope**(指名轨 / 指定范围)一律不前移。落盘见 STATE_SCHEMA §三(CFGS,容器 abi 2→3)与 `docs/contract-changes/20260905-sl279-applied-analysis-settings.md`。 |
...
| 返回 | `{ armed:bool, tracksMask:u16, startS:f64, endS:f64, reason?: "noTracks"\|"noSelection"\|"readOnly"\|"noTimeline" }`(`armed=false` 时必带 `reason`);或 `{ok:false, reason:"badArg"}`(给了 `autoStop` 却不是严格布尔 —— 此时**不撤防、不改 state**,与上面四个 `reason` 不是同一形态) |
...
| 拒绝态 | `armed:false` + `reason`(见上);只读观察态 → `reason:"readOnly"`;宿主不给时间线(判据同 §5.1 `noTimeline`)→ `reason:"noTimeline"`。判序:`noTracks` → `noSelection` → `readOnly` → `noTimeline`。**四条拒绝路径都先走一次撤防**(与「撤销布防」「越界自动停」同一段代码):否则上一次布防若是我们替用户开的采集,这一发被拒之后采集会一直开着、门控还留在旧选区上,没人再去撤。 |
| 撤销 | 否 |
| 线程/频率 | [M];用户操作触发 |
...
| 语义 | 首帧全量快照并置 `mBridgeReady=true`。`channel_id` 报的是**配置(选中)的通道号**(未选过通道时它本就是 `0`);**此外**只在 claim 请求被拒或段不可用时为 `0` —— 那几态下本实例一个 slot 也没持住,报配置号会把被拒的通道显示成「已选中」;这几态下「实际持有」恒为 `0` 是 `InputSession::openAndClaim()` 的结构性保证(每条失败分支要么从未写过、要么显式写 0),**本条据此写成对外承诺** —— 改那里的人破的是本条款而不只是一个内部约定(变更文档 `docs/contract-changes/20260921-sl464-channel-id-zero-semantics.md`)。所以 `channel_id=0` 有两个成因,**分辨它们一律以 `claim`(§4.1/§5.2)为准,不得由 `channel_id=0` 反推「未分配」**:① `claim="unassigned"` —— 从未选过通道 / 主动释放(不 claim 任何 slot,J01,**不是错误态**);② `claim` 取 `conflict`(同组同 channel 被心跳新鲜的实例占用)、`abiMismatch`(abi 不符被拒连)、或 `idle` 中「**段不可用**」那一支(`kUnavailable`,四项:段打不开 / 映射失败 / `claimInput` 非 `kConflict` 的失败 / `createSegments` 失败;逐字取 `src/input/InputBridgeLogic.cpp:39-40` 头注) —— **claim 失败态,不是 J01 的引导态**。两个成因下本实例都未占用本组任何 `InputSlot`,所以两者共有的那一半仍然成立:Output 侧完全不可见该实例、不计入 `N/15` 计数。反向不成立:`claim="unassigned"` 时 `channel_id` **未必**为 `0`(会话已释放而配置号原样留着时,它就是那个配置号)。**口径区分 —— 这是三个量,别混读**:本字段 = **配置镜像**;实际占住 `InputSlot` 的那个号是另一个量(不上桥面,只经上述例外间接体现为 `0`);**工程存档**里的 `channel_id`(见 `docs/STATE_SCHEMA.md` §二 Input state)是第三个量 —— [SL-446] 起三者分开记。**它们会分叉,且分不分叉取决于补偿式回滚成不成功**:点了一个正被占用的通道、又没有可回滚的旧通道(或回滚也失败)时,桥面本字段为 `0`、而工程存档记的是**那个被拒的号**;回滚成功时会话留在旧通道上(`claim` 仍是 `active`),桥面与存档都是**旧号**、不分叉。`config` 为本组 ctrl 广播区中本 channel 的只读快照(Output 离线时字段可为默认值,UI 侧隐藏远程摘要行)。`config.channelLabels` 供 4×4 通道网格首帧渲染(15 张卡的 Output 侧 label,A-32,见 §4.3)。`conn.occupiedMask` 见 §4.2 —— 供 4×4 通道网格在点击前标出已被占用的卡(05 §3)。**键名拼写与 §4.1 `scvb.state` 逐字一致**(A-30,§0.2 规则①)。**`ui.guide_seen`**([J81]/J80/T48)= Input 首启轻量引导已读位,随工程持久化;**`guide_seen_global`** = 系统级全局默认判定位(J50a),**只读、不属工程 state**。首启判据两侧同构:**工程 `ui.guide_seen === false` 且 全局 `guide_seen_global === false`** 才弹。**Input 与 Output 的全局位各存一份**(`input.*` / `output.*` 分键,`UiDefaultsStore` 命名空间本就按侧分)——两侧引导讲的是两个界面、两套内容,共用一个位会让先装 Output 的用户永远看不到 Input 的引导,而那正是 J80 立 T48 的全部理由。 |
| 拒绝态 | 无 |
| 撤销 | 否 |
| 线程/频率 | [M] 同步 |
...
```json
{
  ...
  "output": {
    "functions": [
      ...
      {"name": "recaptureArm", "params": ["tracksMask", "startS", "endS", "autoStop"], "returns": "{armed,tracksMask,startS,endS,reason?} | {ok:false,reason:\"badArg\"}"},
...
| `scvb.state.features.bytes` | 05 §2.4 存储状态显示「内嵌于工程(x.x MB)」 | 特征数据字节数;**v1 收起**:该行随「sidecar 不上」的用户裁定(2026-09-14)一并隐藏,字段仍在事件里下发、仅无 UI 消费面(见 `docs/contract-changes/20260915-sl415-sidecar-ui-hidden.md`) |
