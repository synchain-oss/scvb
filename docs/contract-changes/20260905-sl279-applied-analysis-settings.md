...
同族、方向相反。代价写明:范围档下点完「分析(全部)」注记仍亮,**唯一能让这一档真正落到整条时间线上的出路,
是把 `global.range` 切回 `follow` 再分析**([SL-348] 措辞收紧:把当前档**改回 `applied` 的那个值**
同样会让注记灭 —— `analysis_settings_stale` 是逐项相等比较、`syncStale()` 每帧都在算 —— 但那是
**放弃这次改档**,不是让它生效;上下文里的「出路」指后者,所以写全限定语,别只写「唯一出路」)。取这条是因为注记的语义是「段表未按当前档分析过」—— 范围档下它亮着是**真话**。
