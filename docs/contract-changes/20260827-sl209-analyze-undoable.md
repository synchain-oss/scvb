...
**`analyze` 只置亮 undo、不清 redo**(#152 第三轮复审【重要】):web 侧 `historyAfterSegments`
对其余三类入栈 reason 会把 redo 钮置灰(它们必然压一条事务 ⇒ redo 栈必然被清,置灰**有证据**);
`analyze` 是唯一的例外 —— C++ 侧对空转分析有守卫、不压事务、redo 栈还在,而段表事件里**没有**
「这一轮到底压没压步」的证据。若照着清 redo:空转分析后 redo 栈明明还在钮却灰了,**而灰掉的钮
点不动**,拿不到那次会救回它的 `ok:true` 回执 —— 不可自愈。按 `tab-master.js` 头注既定原则
(「置灰会挡住真实可用的动作(真错),常亮的代价只是白点一下、回执把它置灰(可自愈)」)取留亮。
代价:产出型分析后 redo 钮会多亮一会儿,点一下拿 `ok:false` 即自愈 —— **拿可自愈的假亮,
换掉不可自愈的假灰**。用例 `smoke-undo-redo.mjs` 两条(★反向:删掉 analyze 那一支 → 3 项红;
从白名单摘掉 analyze → 2 项红)。
⚠ 仍未堵的同族:「产出与旧表逐字节相同」「产出全部与保留段 clash 被丢弃」也是恒等变换却照压
一条事务。彻底解要么给 §2.8 加一条「本轮是否入栈」的证据字段,要么让 C++ 侧按**整表相等**判空事务
—— 两条都超出本卡范围,登记在此。
