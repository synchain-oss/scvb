...
### `contractVersion` 保持 `1.0` —— 本卡不升,已转 SL-468

按 §0.1 规则 3(`:15`),「**改既有字段语义**」在禁止面上,而 `channel_id` 在三个 claim 态上的
取值语义**确实变了** —— **该面已由 #273 触及**。本 PR 只做文字追认;是否升主版本号已转
**[SL-468]** 交用户裁定。
...
**合并前置**:本节需由**用户**在 [SL-468] 上二选一 —— **升 2.0** 或
**豁免 §0.1 规则 3 + 写明理由** —— **落定之后本 PR 方可合并**(采纳 #275 复审【红旗】的收口
建议;二选一是用户的选择,不是统筹的)。实际上这也已由流程挡住:本 PR 挂
`status/frozen-contract`,按 CLAUDE.md §3 / §5 只有用户能合。
⚠ 而且**这一版没有任何机检兜底** —— `branch-gate` 在 base=`feature/v1` 的子 PR 上不跑
(见上文「guard 的覆盖面与『本 PR 会不会触发它』是两件事」),所以这一条没有机器会替谁拦。
...
⇒ **按 §9.0 第 3 条读不触发,按 §0.1 规则 3 读触发**,两条读出来结论相反。要不要统一措辞,
由 SL-468 一并定。
...
- **统筹裁定**(2026-09-21):不给新 A/J 号(最近六张契约变更卡 SL-395/398/411/413/414/415/416
  ...
  (转 SL-466);§3.4 不动(转 SL-467);`contractVersion` 保持 `1.0`(转 SL-468 交用户裁)。
...
- 转出的卡:**[SL-466]**(§5.2 `idle` 行)、**[SL-467]**(§3.4 拒绝态口径)、
  **[SL-468]**(`contractVersion` 是否升 2.0 + §0.1 规则 3 与 §9.0 第 3 条措辞不一致)。
