...
取值语义**确实变了** —— **该面已由 #273 触及**。本 PR 只做文字追认,**不替 #273 追认
`contractVersion`**。是否升主版本号已转 **[SL-468]** 交用户裁定。理由三条:

1. **触发点在 #273,不在本卡。** 语义是 #273 改的,它合并时没升版本号,这笔账当时就欠下了;
   一张纯文字追认卡没法回溯地替上一个 PR 补版本号。
2. **改动面。** 升 2.0 要按 §9.0 第 3 条连带更新 mock 后端 + C++ 常量表 +
   `check-bridge-parity.mjs`,本卡会从 3 个 md 涨成跨 `web/` + `src/` + `scripts/`。
3. **授权范围。** 用户批的是「SL-464 契约文字追认」;主版本号 +1 是**对外的破坏性声明**,
   超出那份批准。
