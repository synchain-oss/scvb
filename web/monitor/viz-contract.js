 ...
 *
 * `VizTrackState.panNow` 是**播放头此刻**的值;`VizLanes.pan[t][headCol]` 是**列中心**的点采样。
 * 两者在列内会差一点(列宽 = 窗口跨度 / 1024,300s 工程下约 0.29s),放大档下差得更明显 ——
 * 列中心甚至可能落在**另一段**里。
 *
 * 分布图要的是「此刻」⇒ **标量优先**;标量是哨兵或整块缺失时才回落到车道列。
 *
 * [SL-363] 写方那一侧的标量现在是**分布图读回链**的结果(与 Output 那张图逐条同源):
 * 冻结维 → 参数面;手动常值段 → 该段;输出 ON → 播放头**所在段**的段值(空隙里保持前一段);
 * 输出 OFF → 参数面;取不到段值而该轨**已连接** → 该版本该轨的参数当前值([SL-361]);
 * 其余(未连接 / 参数句柄未就绪)→ 哨兵。
 * 也就是说「标量是哨兵」只剩「未连接 / 参数句柄未就绪」两种,不再包含「这轨没分段」。
 * ⚠ 它**不再是** `CurveEvaluator` 的精确时刻求值 —— 段边界的 ramp 与空隙的切换只在**车道**上有,
 * 分布图那根柱不跟着 ramp 走(理由见 `src/core/output/DistReadback.h` 的头注)。
 * 口径真源在 `docs/IPC_CONTRACT.md` §6.1 那条 ⚠ 里,这里只是镜像,别在这儿改真源。
 * 反过来写(车道优先)在放大档下会让分布图的柱与轨迹图的播放头位置对不上,
 * 而这是「看起来完全正常」的那类错。
 */
export const PAN_NOW_PRIORITY = Object.freeze([
    "trackPanNow", // ① 段里的「此刻」标量(读回链的结果)
