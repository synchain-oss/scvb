 ...
 *
 * [SL-361] 写方那一侧的口径**扩了一档**(读方这张表不变,但要知道标量现在是什么):
 * `panNow`/`volDb` = ① 有段有曲线 → 曲线求值;② 无段/无曲线**且该轨已连接** →
 * 该版本该轨的**参数当前值**(与 Output 分布图的回落同口径);③ 其余 → 哨兵。
 * 也就是说「标量是哨兵」现在只剩「未连接 / 参数句柄未就绪」两种,不再包含「这轨没分段」。
 * 冻结口径在 `docs/IPC_CONTRACT.md` §6.1 那条 ⚠ 里,这里只是镜像,别在这儿改真源。
