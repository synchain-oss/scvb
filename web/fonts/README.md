...
本次生成(2026-09-02,[SL-276] 的「稍后」按钮带进「稍」、[SL-273] 的响度换档影响面文案
更正再带进「材 派 素」,共 4 个新汉字)的规模:拉丁 **153** 字符,CJK **772** 字符,合计 925。
三语文案上千条,靠手抄字表必然漏字,故文案一改就重跑脚本,不再维护第二份字符清单。

> **[SL-396] 2026-09-11 原地重跑**(#256:v5.6.13 收口新增的两条 analyze 拒回执词条带进了
...
> 的 `build_charsets()` 对 **base(`76ffb04`)的 `web/`** 与**当前 `web/`** 各跑一次(两棵树各带一份
> 脚本,ROOT 由脚本自身路径推出;`git archive --format=tar 76ffb04 web web-preview | tar -x -C <临时树>`
> 导出 base,**不联网、不写 .git**)⇒
...
> base = load('base', r'<base 临时树>\scripts\fetch_fonts.py')
...
> 那一版对拍。**读数出处**:当次实跑用的是一次性脚本(不在仓内,`build-*/` 被 `.gitignore` 挡着);
> 复现请照下面这段内联片段,旧版字节走 `git show HEAD:web/fonts/<文件>`:
>
> ```python
...
>     old = TTFont(io.BytesIO(subprocess.run(['git', 'show', 'HEAD:' + rel],
...
> 这也解释了为什么本批三款拉丁 WOFF2 的字节数一个没动:拉丁侧的所需字符集根本没变。
> 逐字比对上一版与本版的两个子集:拉丁侧 added / removed 双向皆空,CJK 侧 added 恰为
> 「稍 材 派 素」四个 —— 与上面那句话对得上。
...
| `Noto Sans SC`          | 下载上游全量可变字体,本地 `fontTools` 子集化        | CJK 集当前 767 字(随文案增删,有增有减),`text=` 的 GET URL 会撞上限              |
...
SCVB 的 767 个 CJK 字会大面积缺字 —— 走回退期间 **gates 3h 会一直红**,这是刻意的:
...
所需 **920** 个字符(拉丁 153 + CJK 767)全部有字形 —— 白名单那 2 个除外
(`⇕` `✕`:四款上游家族本身就没有,由宿主系统字体上屏,见上文那一节)。
拉丁三款按设计缺 `⇒ ① ② ④ ⓘ ⚠`(逐字回退到 Noto,`[INFO]` 不判红)。
