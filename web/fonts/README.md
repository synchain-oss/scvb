...
本次生成(2026-08-16,T27 第二波 findings 修订后重跑)的规模:拉丁 136 字符,CJK **404** 字符。
三语文案上千条,靠手抄字表必然漏字,故文案一改就重跑脚本,不再维护第二份字符清单。
(上一次是 CJK 396;i18n.js 补了 tour demo 的 15 条轨名 `demo.ch1..15` 后多出 8 个汉字
`低 八 吸 呼 和 尾 念 群`——不重跑就会在 tour 里上屏方块,而 CI 查不出,正是本 README 说的那种漏字。)
...
SCVB 的 404 字会大面积缺字,**T31 前必须联网重跑 `fetch_fonts.py` 替换**。

当前四款均为**联网实跑生成的 SCVB 真子集**,未走回退;已用 `fontTools` 校验字符覆盖:
Noto 对 540 个请求字符缺 0,拉丁三款仅缺 `⚠`(见上,按设计走栈回退)。
