...
//     **[SL-415] 2026-09-14 用户裁定 sidecar 不上了 ⇒ 「存储状态」那一步整步移除,44 → 43 步**,
//     见下面步骤表里那一处注释;05 §2.6 的步数由统筹在 masterPlan 侧同步);
...
/**
 * tour 步骤清单(用户 2026-08-23 指令:7 步基线作废,扩为全参数导览;每参数一步,
 * 含 T33 波形页控件;05 §2.6 待统筹勘误,deviations 登记)。
 ...
 * 增删步须先改 05 §2.6,再回改本表(05 §2.6「增删步须回本表改」)。
 *
 * ⚠ **[SL-415] 本表是 43 步,不是 44**。原第 42 步(`{anchor:"storage"}` —— 设置页
 * 「存储状态」)由用户 2026-09-14 裁定「sidecar 不上了」后**整步移除**:那张卡片在
 * `index.html` 里恒挂 `hidden`,spotlight 锚过去只会挖出一个空处(蒙版打在零尺寸盒上,
 * 说明框指向一片没有内容的区域)。**不是「改个锚点」,是这一步没有了** —— DOM 里的
 * `data-tour="storage"` 属性按「隐藏 ≠ 删除」留着(将来要恢复这一步,把它插回原位、
 * 并把 42 之后的词条整体往后挪一格即可)。
 *
 * ⚠ 移除一步会**连带挪动后面每一步的编号** —— 文本是按序号取词条的
 * (`updateText()` 里的 `"tour.step" + step + ".title"`,见那一处),所以本表一改,
 * `web/shared/i18n.js` 里 `tour.step42/43/44.*` 三语必须同步重编号
 * (诊断 → 42、重看引导 → 43,原 42 的存储状态两条删除)。漏掉这一步的后果不是「少一句」,
 * 而是**诊断那一步显示着存储状态的说明**——一句屏幕上无处可指的假话。
 */
export const TOUR_STEPS = Object.freeze(
    [
        ...
        // ⚠ 原第 42 步 `{ anchor: "storage", tab: "settings" }` 在这里 ——
        // **[SL-415] 用户 2026-09-14 裁定「sidecar 不上了」,整步移除**(那张卡片已收起,
        // spotlight 会指到空处)。删掉它 = 后面每一步的序号各前移一格,所以
        // `i18n.js` 的 `tour.step42/43/44.*` 三语同批重编号 —— 理由与漏掉的后果见上方
        // 步骤表头注。
        { anchor: "diagnostic", tab: "settings" }, // 42(原 43)
        { anchor: "review", tab: "settings" }, // 43(原 44)
    ].map(Object.freeze),
);
