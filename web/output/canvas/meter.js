...
//
// 【SL-191 → SL-204 用户裁定 2026-08-27】CSS 侧 `.sc-tube__liquid` / `.sc-tube__peak`
// 的过渡时长由 `--dur-meter`(.18s)改成 `--dur-meter-tube`(**33ms = 一个 §2.5 事件
// 间隔**),渲染在两帧内追平本模型。
// 原先留着 .18s 的理由写的是「30 Hz 事件之间的帧间平滑,与本弹道叠加不冲突」—— 这条
// 站不住:.18s 是事件间隔的 5.4 倍,它不是补间,是**叠在本弹道之上的第二条更慢的弹道**,
...
// SL-191 先整条去掉(渲染逐帧 == 本模型),SL-204 按用户「软化起跳」的裁定改取 33ms:
// 柱头照样触得到线,而上行摊到 2 帧、不再是单帧跃迁。
// 实测对照表、量尺与反向验证见 base.css `.sc-tube__liquid` 上方的行注。
// 补间跨度不得再长过一个事件间隔 —— 更长就是在补一个已经过期的目标;
// 其余帧间平滑本来就归本文件管:rAF 60fps 逐帧按 dt 推进,下行 fast-follow 天然连续。
//
