...
export function createWaveformSource(opts) {
    ...
    return {
         ...
         * 附各自的时间范围。
         *
         * 用途:视口变化中的过渡帧**挑源**。调用方(tab-wave `paintStaticLanes`)
         * 从这份候选里只选**一块**画满整幅 —— 先滤掉盖不住当前视口的,再按列宽
         * 最接近 1px(最像稳态)择优;一块都选不出就退全曲概览块。
         * 曾经的用法是把几块按时间**拼**起来补满视口,已废弃:各块是不同时刻/
         * 不同档位的快照,列宽差一倍观感就差一截,拼出来是一片深浅不一的补丁
         * (用户实测「亮块暗块,动画一停马上消失」)。序在单源下已无意义,
         * 保留窄→宽只是让调用方的择优循环先遇到细节块,早收敛。
         *
        ...
        peekOverlapping(ch, startS, endS) {
            ...
            // 窄→宽、新鲜组在前。单源口径下这个序不再决定画面(调用方只取一块,
            // 且按列宽择优),只影响择优循环的收敛顺序与下面 CAP 截断留下谁。
            // pick 给的是宽→窄,各自 reverse 成窄→宽,新鲜组整体排在前。
            const fresh = pick(perCh.get(ch)).reverse();
            const stale = pick(perChStale.get(ch)).reverse();
            const all = fresh.concat(stale);
            // 封顶:本函数每帧 × 每条可见泳道跑一次。单源下不再逐块绘制,截断
            // 省的是候选遍历与 tile 数组的持有面;窄→宽取前 N 个即可(细节优先)。
