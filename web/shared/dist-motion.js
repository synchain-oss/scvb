...
//   • Monitor(`web/monitor/app.js`)读 viz 共享段,契约 §6 **4Hz**(冻结口径,不许改)。
//
...
// 是:新帧到达时把**当前显示值**当起点、新值当终点,在**一个发布周期**内走完 ——
// 落点恰好踩在下一帧的预期到达时刻上,平均滞后 = 半个发布周期,且**永不越过目标**。
//
...
/**
 * 补间时长(ms)= viz 段的**发布周期**(契约 §6:帧头 4Hz,冻结口径)。
 *
 * 为什么是这个数、而且是个**定值**(而不是看着更聪明的「实测帧间隔」):
 *
 * 新值出现在画面上时,我们唯一知道的事实是「它发生在最近一个发布周期之内」——
 * 上一帧还没有它,这一帧有了。那个 250ms 就是这次变化**可能占据的全部时间窗口**,
 * 也就是唯一有依据的重建窗口。
 *
 * 而「实测间隔」量的是另一件事:**数据多久变一次**。引擎的段内取值是常量,过渡走 ramp
 * (`CurveEvaluator`:80ms 起,受 15 pan/s、3 dB/s 限速,空隙处最长 6s)—— ramp 期间几乎
 * 每帧都在变(实测间隔 ≈250ms,与定值一致),而两次 ramp 之间可以好几秒不变。拿那个
 * 好几秒去当补间时长,会让一次孤立的段边界跳变被抹成一段慢吞吞的爬行 —— 那是**编出来的
 * 运动**,时长与真实变化毫无关系。
 *
 * 另一个不容易看见的好处:定值不受「谁调用了 render」影响。`scvb.groups`(1Hz)与
 * `scvb.state` 也会触发一次 render → 一次 push,若拿「上次 push 到现在」当间隔,
 * 这些与数据面无关的事件会把测量搅乱,而画面上只表现为运动时快时慢。
 */
export const DIST_SPAN_MS = 250;
