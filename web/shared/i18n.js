...
export const T = {
    zh: {
        ...
        // ⚠ **本条现在回到了 v5.6.6 的原句**(J97 / 用户裁定 2026-09-06):
        //   用户实测 v5.6.7 后明确说「是否参与音量调节为独立开关,不随此联动」这句**不要加**。
        //   下面那段 2026-08-21 的口径警示对「参与语义」本身仍然成立(列头那一侧看
        //   `tracks.colLegend`,它**没有**被本次裁定动),但**不再适用于本条** ——
        //   这句提示只说「强制居中」,独立开关那层意思由列头说明承担。
        //   两条用户裁定在这一处相抵时,**晚的那条赢**。
        ...
        // follow 变体与上面 `out.master.writeConfirm` **措辞刻意保持一致** —— 这是「会覆盖
        // DAW 已有自动化」的那句警告,用户在两种范围模式间切一下,不该看到它换说法。
        // 两条同动,en/fr 三语同理。
        //
        // ⚠ **真源是用户裁定,不是 05 §5**(J97 / 用户裁定 2026-09-06):
        //   用词是「轨道 / 已激活 / 不保存」(en `active`、fr `actif`),**不是**「车道 / arm /
        //   落盘」。SL-293 曾按 05 §5 把两条改成后者,用户实测 v5.6.7 后明确退回:
        //   「不如以前的,保留轨道、激活和保存」。05-ui-spec 对应句子已随本裁定改。
        //   **别再照规格改回去** —— 那正是本卡在修的那次(「晚出的用户裁定没有 J 号」那一族:
        //   规格停在裁定之前,照规格改等于把用户已经拍过的板推翻)。
        ...
        // 乐句(1–4s)下拖动本杆看不到变化。
        // [SL-382] native 侧已评估完,结论比这句更糟:谷切分那一步收到的是**线性 kw**、
        // 判据却按 dB 定标,于是整个 0..100 量程恒不切 —— 那一层已修(`AnalysisPipeline.cpp`)。
        // 但 S1 的边界被 02 §3.4 步骤 4 合并回去、进不了段表,所以任何长度上段数都不变。
        // ⇒ 用户裁定(2026-09-10):**这个功能暂时不做,先把滑杆藏起来**
        //    (`web/output/index.html` 的 `data-gb="wave-seg-sensitivity"` 挂 `hidden`)。
        // **本条与 `wave.sldSensitivity` 三语六条词条一律保留、不删**:①隐藏 ≠ 删除,
        //   `SLIDERS` 里那条仍是 §1.19 整包下发的底账;②`scripts/check-i18n.mjs` 要三语 key
        //   全等,删一处就得三处一起删,而这功能是**暂缓**不是废弃。
        // ⚠ 词条眼下**没有渲染出口**(控件藏了)。放杆回来之前必须先改这句 —— 它现在暗示
        ...
        // 之后,挂在一起就是假句)。
        "wave.tipMinSeg":
            "最短段长:短于它的自动段会被丢掉(在前后留白之前判定)或并入相邻段,用于过滤杂音,但也会丢掉短促的 ad-lib 与单字和声;手动段不受影响。整条时间线重新分析后段表里不再有短于它的自动段,选区或范围档重分析在窗边可能留下更短的残段(默认 {d})",
        ...
        // 这条闸现在的口径与理由都换了:[SL-399 R34] **VAD 那一级有最小长度闸(「丢短」),
        // 而写回裁剪没有**(`OutputProcessor.cpp` 的 R8),所以给这么短的段点「恢复自动」
        // **最多只留下一条裁到段内的残段**(窗内静音时则是被清掉、什么都不留)⇒ 不给入口。
        // 与锁定段同一处理:说清楚 + 给出路。
        "wave.restoreSegTooShort":
            "这一段短于最小分段时长 {n} ms,不提供「恢复自动」:重算只会写回这一段的范围,最多留下一条同样短的残段;请把「最小分段」调小,或先合并相邻段。",
    ...
    en: {
        ...
        "wave.tipMinSeg":
            "Minimum segment length: automatic segments shorter than it are dropped (judged before padding) or merged into a touching neighbour to filter noise, but it also drops brief ad-libs and single-note harmonies; manually edited segments are unaffected. After a full-timeline re-analysis the segment table no longer contains automatic segments shorter than it; scope or range re-analysis may leave shorter stubs at the window edges (default {d})",
        "wave.emptyMain":
        ...
        "wave.restoreSegTooShort":
            "This segment is shorter than the minimum segment length ({n} ms), so Restore automatic is not offered: re-analysis only writes back within this segment and would leave at most an equally short fragment. Lower “Min segment”, or merge it with a neighbour first.",
        "wave.clearCoverageConfirm":
    ...
    fr: {
        ...
        "wave.restoreSegTooShort":
            "Ce segment est plus court que la durée minimale de segment ({n} ms) : le rétablissement automatique n’est pas proposé, car le recalcul n’écrirait que dans ce segment et ne laisserait au mieux qu’un fragment tout aussi court. Réduisez « Segment min » ou fusionnez-le d’abord avec un voisin.",
        "wave.clearCoverageConfirm":
