...
export function hostEchoVisible(store, nowMs) {
    ...
    // ① 本次播放里写过 ⇒ 常亮(无窗口)。
    //
    // ⚠ 门限是 `start - 去抖窗` 而**不是**裸的 `start`:`scvb.params` 与 `scvb.playhead`
    // 是**两条独立的事件流**,谁先到达没有保证。用户按下播放的那一刻宿主就开始写,
    // 而本段播放的起点要等第一帧 `playhead` 才落下来 —— 裸 `start` 会把「早到几十毫秒
    // 的那一笔写」判成「上一段播放的事」,于是**每次起播都先灭一下**,直到宿主下一笔写
    // 才亮。用户裁定里「不闪」那一条当场破。
 ...
 * 落点:记反了(停走帧也刷新)窄档就**永远进不去**,停播后的熄灭退回**播放档**
 * (smoke-tab1 ⑧(a19) 注入实测 2520 / 2220 / 1620ms,而不是 900 内)—— 就是 SL-270
 * 修掉的第一幕「停播后挂着近两秒」原样复活,而它在纯函数层只隔一行。
 * ⚠ 是「熄得晚」不是「永不熄」:`hostEchoOn` 那一头的播放档窗口照旧会到期。
 *
